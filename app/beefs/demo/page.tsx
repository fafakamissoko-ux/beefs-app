...
export default function ArenaLobbyPage() {
  ...
  const [userBalance, setUserBalance] = useState(150); // Mock user balance - will be fetched from DB
  ...
  const handleCreateBeef = async (beefData: any) => {
    ...
    try {
      const insertData: any = {
        ...
        subject: beefData.title, // subject mirrors title (NOT NULL)
  ...
  const purchaseAccess = async () => {
    ...
    if (userBalance < selectedRoom.price) {
      toast('Solde insuffisant! Rechargez vos points.', 'error');
      return;
    }

    // Deduct points
    setUserBalance(userBalance - selectedRoom.price);
    
    // Add user to participants (in real app, update database)
    // await supabase.from('rooms').update({ participants: [...participants, userId] })
    
    setShowPaymentModal(false);
    // NEW: Use /beef for modular system
    router.push(`/beef/${selectedRoom.id}`);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          >
            <div>
              ...
              <p className="text-gray-400">
                {feedType === 'pour-vous' 
                  ? 'Découvre les beefs les plus captivants du moment' 
                  : 'Beefs des personnes que tu suis'}
              </p>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <div className="text-6xl mb-4">{feedType === 'pour-vous' ? '🎭' : '👥'}</div>
              ...
              <motion.div
                ...
                className={`group bg-gradient-to-br rounded-2xl p-6 border cursor-pointer transition-all ${
                  room.is_premium
                    ...
                    : 'from-gray-800 to-gray-900 border-gray-700 hover:border-pink-500 hover:shadow-2xl hover:shadow-pink-500/20'
                }`}
              >
                ...
                {/* Title */}
                <h3 className="text-xl font-black text-white mb-2 line-clamp-2 group-hover:text-pink-400 transition-colors">
                  {room.title}
                </h3>
          ...
          >
            ...
            >
              ...
              <div className="space-y-3 mb-6">
                ...
                {userBalance >= (selectedRoom.price || 0) && (
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={userBalance < (selectedRoom.price || 0)}
                  className="flex-1 bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 text-black font-bold py-4 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg disabled:shadow-none flex items-center justify-center gap-2"
                >
                  <Lock className="w-5 h-5" />
                  {userBalance < (selectedRoom.price || 0) ? 'Solde insuffisant' : 'Acheter l\'accès'}
                </button>
              </div>

              {userBalance < (selectedRoom.price || 0) && (
                <p className="text-red-400 text-center text-sm mt-4">
                  💡 Rechargez vos points pour accéder à cette salle premium
                </p>
              )}
            </motion.div>
          </motion.div>

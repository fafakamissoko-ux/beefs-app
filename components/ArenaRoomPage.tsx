...
export function ArenaRoomPage({ roomIdParam, roomId }: ArenaRoomPageProps) {
  ...
  useEffect(() => {
    ...
    const loadRoomData = async () => {
      ...
      const fp = (beef as { free_preview_minutes?: number }).free_preview_minutes;
      setFreePreviewMinutes(typeof fp === 'number' ? fp : 10);
      setContinuationPricePoints((beef as { price?: number }).price ?? 0);
      setPreviewStartedAt((beef as { started_at?: string | null }).started_at ?? null);

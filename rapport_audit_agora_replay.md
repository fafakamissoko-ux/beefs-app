# Rapport d'audit source — Agora Live & Replay (Tier-1)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** préparer la mise à niveau Tier-1 de l'Agora en direct et le cycle de vie post-live (Replay).
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `app/arena/[roomId]/page.tsx` | 466 | Route arène : auth, billet Daily, sas matériel, écran beef terminé → summary |
| `components/TikTokStyleArena.tsx` | 4325 | **Monolithe arène** : Daily, layout, chat, régie, aura, timers, `useArenaRealtime` |
| `hooks/useArenaRealtime.ts` | 773 | Canaux Supabase Realtime + broadcast + polling secours |
| `components/BeefCard.tsx` | 1006 | Carte feed : statuts live / scheduled / ended / replay |

**Routes connexes (non extraites) :** `app/live/page.tsx` (362 L), `app/live/[id]/page.tsx` — délèguent aussi à `TikTokStyleArena`.

---

## 1. Architecture Agora Live

```
app/arena/[roomId]/page.tsx
  ├─ FETCH_TICKET → beefs + beef_participants + fetchBeefVideoTicket
  ├─ beef.status ∈ { ended, cancelled, replay } → écran « Beef terminé » + lien /beef/{id}/summary
  └─ TikTokStyleArena (Daily URL + token + userRole)
        ├─ useDailyCall (WebRTC)
        ├─ useArenaRealtime (Supabase channels)
        ├─ ArenaLayoutManager (Nexus / Constellation)
        └─ ~50+ useState (régie, aura, timer, chat, verdict…)
```

### `TikTokStyleArena` — gestion d'état (extrait)

| Domaine | États clés |
|---------|------------|
| Join / Daily | `hasJoined`, `showPreJoin`, `preJoinMediaStream`, `participantRoles` |
| Aura / heat | `auras`, `auraMed`, `globalHeat`, stores Zustand `arenaPulseVoices`, `arenaVerdict` |
| Timer beef | `beefTimeRemaining`, `timerActive`, `DEFAULT_BEEF_DURATION` (60 min) |
| Parole / débat | `speakingTurn*`, `structuredDebateEnabled`, `mediatorHoldingFloor` |
| Fin de beef | `beefEnded`, `endSummary`, callbacks `onBeefVerdict`, `onBeefEnded` via realtime |
| UI | `mediatorSidebarOpen`, `visibleMessages`, `flyingReactions`, `isCinematicMode` |

**Hook réseau :** `useArenaRealtime` appelé ~L3031 avec callbacks mappés sur setState arène.

---

## 2. Moteur temps réel (`useArenaRealtime`)

### Canaux Supabase

| Canal | Type | Deps effect | Cleanup |
|-------|------|-------------|---------|
| `live_${roomId}` | **broadcast** (18 events) | `[roomId, flushBroadcastOutbox]` | `removeChannel` + cancel reconnect timer |
| `spectator_invite_sync_${roomId}_${userId}` | postgres_changes UPDATE `beef_participants` | `[roomId, userId]` | `removeChannel` |
| `beef_participants_live_${roomId}` | postgres_changes * `beef_participants` | `[roomId]` | `removeChannel` |

### Events broadcast `live_${roomId}`

`sfx`, `reaction`, `aura_batch`, `aura_master_sync`, `message`, `delete_message`, `arena_big_gift`, `pulse_voice`, `announcement_banner`, `beef_global_timer`, `speaking_turn`, `mediator_floor`, `mediation_toss`, `structured_debate`, `mediator_mute_challenger`, `beef_verdict`, **`beef_ended`**

### Reconnexion & file d'attente

- Debounce init **150 ms** (Strict Mode)
- Outbox broadcast (`BROADCAST_OUTBOX_CAP = 48`) si canal non SUBSCRIBED
- Reconnexion auto **3 s** après `CLOSED` / `CHANNEL_ERROR` / `TIMED_OUT`
- `callbacksRef` : callbacks parent mis à jour sans re-mount canal

### Polling secours (8 s)

- `beef_messages` → `onMessageReceived` (source `poll`)
- `beef_reactions` → `onReactionReceived` (source `poll`)

---

## 3. BeefCard — statuts `status`

### Type union (L25)

`'live' | 'ended' | 'replay' | 'scheduled' | 'cancelled' | 'pending' | 'ready' | 'completed'`

### Dérivations visuelles

| Variable | Logique |
|----------|---------|
| `isReplay` | `ended \| replay \| completed` |
| `statusVariant` | live → ring cyan ; pending/scheduled/ready → ring blanc ; ended/replay/cancelled → opacity/saturation |
| `replayHover` | preview vidéo au survol (replay) |

### CTA par statut (footer ~L803+)

| Status | Action UI |
|--------|-----------|
| `live` | « Rejoindre le Direct » / « Retourner dans l'Agora » (`liveAudienceAction`) |
| `scheduled` / `pending` / `ready` | Countdown, notify, saisir affaire, valider Ref, se désister |
| `ended` / `replay` / `completed` | Lecture replay / pas de join live |
| `cancelled` | État terminé atténué |

### Badge live (L295)

Pulsing « EN DIRECT » si `status === 'live'`.

---

## 4. Cycle post-live actuel

| Couche | Comportement |
|--------|--------------|
| `arena/[roomId]/page.tsx` | Si `beef.status` ended/cancelled/replay → écran statique + lien `/beef/{id}/summary` |
| `useArenaRealtime` | Event `beef_ended` broadcast → `onBeefEnded` |
| `TikTokStyleArena` | `beefEnded`, `endSummary` state + overlays verdict |
| `BeefCard` | Affichage replay / opacité réduite ; pas de pipeline replay unifié côté carte |

---

---

## Code source — `app/arena/[roomId]/page.tsx`

*Point d'entrée arène + cycle post-live*
...
## Code source — `components/TikTokStyleArena.tsx`

*Cœur Agora (vidéo + état)*
...
## Code source — `hooks/useArenaRealtime.ts`

*Moteur temps réel Supabase*
...
## Code source — `components/BeefCard.tsx`

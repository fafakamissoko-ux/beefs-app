# Rapport d'audit source — Phase E.3 DOM Audio (soumission iOS Safari)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** préparer l'injection d'une balise `<audio>` physique dans le DOM — remplacement du `new Audio()` mémoire (E.1 rejeté par iOS).

---

## Verdict Architecte — Phase E.3

### E.3.1 — Échec E.1 : `new Audio()` hors DOM

| Élément | Fichier | Problème |
|---------|---------|----------|
| `let dummyAudio: HTMLAudioElement \| null` | `useMediaSession.ts` L9 | Singleton **non attaché au DOM** |
| `new Audio(SILENT_MP3)` | L17 | WebKit iOS ignore souvent les éléments audio non rendus |
| `takeSystemFocus()` | L14-28 | Appelé depuis `TikTokStyleArena.handleJoin` L2759 |
| Cleanup unmount hook | L59-61 | `dummyAudio.pause()` + null — pas de nœud DOM à préserver |

**Conclusion E.3 :** supprimer `dummyAudio` module-level ; exposer métadonnées seules + accepter un `ref` vers `<audio>` DOM.

---

### E.3.2 — Points d'injection PreJoin (`PreJoinScreen.tsx`)

| Bouton | Lignes | Handler | Mode |
|--------|--------|---------|------|
| **👁️ Regarder le Beef** | L203-209 | `onClick={() => onJoin(null)}` | Spectateur |
| **Rejoindre le direct** | L384-390 | `onClick={handleJoin}` → L160-169 → `onJoin(...)` | Participant |

**Emplacements `<audio>` masqué proposés :**
- Juste **avant** le bouton join dans chaque branche (viewer L203, participant L383)
- Ou **une seule** balise en racine du return participant L217 + viewer L181

**`handleJoin` PreJoin (L160-169) :** appelle `onJoin` **sans** `takeSystemFocus` — le geste est relayé à `TikTokStyleArena.handleJoin`.

**Recommandation E.3 :** `audioRef.current.play()` **dans le onClick** du bouton (geste direct), pas uniquement via callback parent.

---

### E.3.3 — Risque démontage PreJoin (`TikTokStyleArena.tsx`)

**Condition de rendu PreJoin (L3148-3150) :**
```tsx
...
**Séquence au join (L2758-2762) :**
```typescript
const handleJoin = (...) => {
  takeSystemFocus();        // E.1 — dummy mémoire
  ...
  setHasJoined(true);
  setShowPreJoin(false);    // ← PreJoin DÉMONTE immédiatement
};
```

| Risque | Impact |
|--------|--------|
| `<audio>` **dans PreJoin uniquement** | **Détruit** au `setShowPreJoin(false)` → flux lock screen coupé |
| `hasJoined && !showPreJoin` | PreJoin absent pendant toute la session live |
| `useMediaSession` dans TikTokStyleArena L248 | **Reste monté** — métadonnées OK, mais dummy E.1 pas DOM |

**Verdict montage E.3 :** remonter `<audio ref={systemAudioRef}>` à la **racine** de `TikTokStyleArena` (persiste après join), **en dehors** du bloc conditionnel `showPreJoin`.

**Candidats racine :** fin du JSX principal (~L4285 près de `<MeetingAudioOutlet />`) ou wrapper `return` top-level.

---

### E.3.4 — Architecture cible E.3 (non implémentée)

```
TikTokStyleArena (toujours monté)
  ├─ <audio ref={systemAudioRef} src={SILENT_MP3} loop playsInline hidden />  ← racine
  ├─ useMediaSession(title, artist, { audioRef: systemAudioRef })  ← metadata only
  └─ PreJoinScreen
        onClick join → systemAudioRef.current.play() (sync, 1ère instruction)
        → onJoin → setShowPreJoin(false)  (PreJoin part, <audio> reste)
```
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `components/PreJoinScreen.tsx` | 396 | Boutons join + geste utilisateur |
| `hooks/useMediaSession.ts` | 68 | E.1 dummy mémoire + metadata |
| `components/TikTokStyleArena.tsx` | ~4387 | Montage/démontage PreJoin |
...
## `hooks/useMediaSession.ts` (67 lignes)
...
## `components/TikTokStyleArena.tsx` (4387 lignes)

# Rapport d'audit — Phase F.2 Hybrid (pré-extraction)

**Date d'extraction :** 2026-06-28 13:27  
**Branche :** `main` (post F.1 — commit `dfef69a`)  
**Mission :** Extraction stricte avant destruction du contexte PreJoin et remplacement oscillateur Web Audio par asset réseau HTMLAudioElement.  
**Contrainte :** Zéro modification du code source — snapshot brut pour l'Architecte.

---

## Contexte Phase F.2

L'approche Web Audio pure (F.1) est étouffée par iOS Safari :
- **Collision de contextes** : `AudioContext` PreJoin (vu-mètre micro) vs singleton système `useMediaSession`
- **Lock Screen Widget** : exige un flux réseau réel, pas un `OscillatorNode` silencieux

Objectif F.2 : destruction séquentielle du contexte PreJoin + singleton `HTMLAudioElement` alimenté par asset statique réseau.

---

## 1. Sas d'entrée — `components/PreJoinScreen.tsx`

**Points d'intérêt Architecte :**
- Ordre join : `onTakeSystemFocus?.()` → `onJoin` / `handleJoin`
- `handleJoin` : `releasePreJoinResources({ stopTracks: false })` appelle `closeAudioContext()` **avant** handoff Daily
- `closeAudioContext()` : ferme le `AudioContext` vu-mètre (l.47–54)
- `startPreview` : crée un **second** `AudioContext` local (l.99–114) — collision potentielle avec F.1
- Prop `onTakeSystemFocus?: () => void` — câblée depuis TikTokStyleArena
...
## 2. Moteur système — `hooks/useMediaSession.ts`

**Points d'intérêt Architecte :**
- Singletons module : `webAudioCtx`, `silenceNode` (OscillatorNode + Gain 0)
- `startSystemAudio()` : `AudioContext` / `webkitAudioContext`, `resume()`, oscillator, `playbackState = 'playing'`
- **À purger F.2** : remplacer par singleton `HTMLAudioElement` + URL asset réseau statique
- Metadata + handlers lock screen (`play`/`pause`)
...
## 3. Arène centrale — `components/TikTokStyleArena.tsx`

**Points d'intérêt Architecte :**
- `const { startSystemAudio } = useMediaSession(...)` — l.247
- `<PreJoinScreen onTakeSystemFocus={startSystemAudio} />` — ~l.3128
- Plus de balise `<audio id="arena-system-audio">` (supprimée F.1)
- Point d'injection F.2 : montage racine arène pour `<audio>` réseau ou ref partagée
- `MeetingAudioOutlet`, `playSfx` (`new Audio()` éphémère)
...
## Synthèse séquentielle (référence rapide)

### Ordre actuel au tap « Rejoindre » (participant)
```
1. onClick → onTakeSystemFocus?.()     [startSystemAudio — singleton système]
2. onClick → handleJoin()
3. handleJoin → releasePreJoinResources({ stopTracks: false })
4. releasePreJoinResources → closeAudioContext()   [ferme contexte PreJoin vu-mètre]
5. handleJoin → onJoin(acquired, { camEnabled })   [Daily handoff]
```

### Collision identifiée
| Contexte | Fichier | Rôle | Cycle de vie |
|----------|---------|------|---------------|
| `audioContextRef` PreJoin | PreJoinScreen | Vu-mètre micro | Créé preview, fermé au join |
| `webAudioCtx` singleton | useMediaSession | Lock screen / silence | Persiste hors React |

iOS Safari : deux contextes + absence de flux réseau → widget lock screen non fiable.

### Cibles destruction F.2
1. Supprimer `AudioContext` + analyser du PreJoin (ou unifier)
2. Remplacer `OscillatorNode` par `HTMLAudioElement` + `/public/...` URL
3. Réinjecter élément audio au montage TikTokStyleArena si nécessaire

---

*Fin du rapport — extraction Phase F.2.*

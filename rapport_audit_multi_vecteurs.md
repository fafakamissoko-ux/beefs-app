# Rapport d'audit — Multi-Vecteurs (Chat/Cadeaux + Spectateurs)
...
## Synthèse diagnostic Multi-Vecteurs

### A. Rendu chat & animation cadeau

| Composant | Zone | Point d'analyse |
|-----------|------|-----------------|
| `ArenaChatMessages.tsx` | L.6–30 | `getUsernameColor` — hash djb2 (5381), 16 classes Tailwind |
| `ArenaChatMessages.tsx` | L.57–65 | Bloc gift : `inline-block` sans `max-w` / `break-words` — pseudo `inline-block mr-2` + contenu inline → **débordement horizontal possible** |
| `ArenaChatMessages.tsx` | L.59 | Pseudo cadeau : couleur hash sur fond ambré — **contraste / collision visuelle** |
| `FullscreenGiftAnimation.tsx` | L.39–51 | Parseur punchline : `replace('{recipient}')` puis `split('{sender}')` — sender en `text-amber-400` |
| `FullscreenGiftAnimation.tsx` | L.43–50 | Conteneur `text-center leading-snug` sans `break-words` — `max-w-[90vw]` sur parent L.98 |
| `gifts.ts` | L.9–23 | 13 entrées `messageTemplate` avec placeholders `{sender}` / `{recipient}` |

### B. Mécanique spectateurs

| Zone (`TikTokStyleArena.tsx`) | Ligne | Comportement |
|----------------------------------|-------|--------------|
| Prop `viewerCount` | L.147, L.229 | Valeur initiale depuis la page (`initialViewerCount` Supabase) |
| State `liveViewerCount` | L.826 | `useState(viewerCount)` — compteur affiché HUD + modale |
| `increment_viewer_count` | L.1534–1543 | `useEffect` si `isJoined && isViewer` : RPC Supabase + `setLiveViewerCount(prev => prev + 1)` local |
| Cleanup | L.1540–1542 | `decrement_viewer_count` au démontage — **sans** `setLiveViewerCount(-1)` local |
| Chaleur globale | L.833–839 | Delta `liveViewerCount` → `setGlobalHeat` |
| `ViewerListModal` | L.3905–3907 | `viewers={remoteParticipants.map(...)}` (Daily) vs `viewerCount={liveViewerCount}` (compteur) — **désalignement liste / compteur** |
| `remoteParticipants` | L.1309 | Provient de `useDailyCall` — peers WebRTC, pas filtrés spectateurs uniquement |

---

## 1. Code source intégral — `components/ArenaChatMessages.tsx`
...
## 2. Code source intégral — `components/Arena/FullscreenGiftAnimation.tsx`
...
## 3. Code source intégral — `lib/constants/gifts.ts`
...
## 4. Code source intégral — `components/TikTokStyleArena.tsx`
...
## 5. Code source intégral — `components/ViewerListModal.tsx`

# Rapport d'audit source — Phase D.2 (Smart Flipper, PiP tuiles, UI overflow)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** préparer l'Ordre de Frappe D.2 — Smart Flipper, pivot PiP tuiles visibles, flex-wrap/truncate UI.

---

## Verdict Architecte — Points d'ancrage D.2

### SmartPiPManager — emplacement réel (correction)

| Fichier | Lignes | Rôle |
|---------|--------|------|
| **`components/Arena/ArenaLayoutManager.tsx`** | L9 import, **L178 instanciation** | `<SmartPiPManager tiles={tiles} activeSpeakerPeerId={...} />` |
| `components/TikTokStyleArena.tsx` | L3523 | Passe `activeSpeakerPeerId` à `ArenaLayoutManager` — **pas d'import SmartPiPManager** |

**Ordre de destruction D.2 :**
1. Supprimer L178 + import L9 dans `ArenaLayoutManager.tsx`
2. Supprimer fichier `SmartPiPManager.tsx`
3. Retirer prop `isSmartPiPUI` de `ParticipantVideo` + adapter `usePiP` pour injection sur tuile active
4. Propager `enableAutoPiP` / `isPiPTarget` via `NexusGrid` / `ConstellationOrbit` → `ArenaVideoSurface` → `ParticipantVideo`

### Chaîne PiP actuelle

```
TikTokStyleArena (activeSpeakerPeerId L1419, L3523)
  └─ ArenaLayoutManager (SmartPiPManager L178)
        └─ SmartPiPManager → ParticipantVideo(isSmartPiPUI=true) → usePiP(ref, true)
```

**Pivot D.2 proposé :**
```
activeSpeakerPeerId → tuile visible → ParticipantVideo(enableAutoPiP = isActiveSpeaker || fallback local)
```

### flipCamera — cible Smart Flipper (L392-399 useDailyMeetingEngine)

```typescript
await callRef.current.cycleCamera(); // À remplacer par setInputDevicesAsync / facingMode
```

### UI — cibles flex-wrap / truncate

| Zone | Fichier | Lignes |
|------|---------|--------|
| `localControls` wrapper | ArenaVideoSurface | L81-127 |
| `pseudoBadge` outer + inner | ArenaVideoSurface | L129-161 |
| `ParticipantVideo` tuile visible | ArenaVideoSurface | L185-190 |

### TikTokStyleArena — ancres PiP / WebRTC

| Symbole | Lignes | Usage |
|---------|--------|-------|
| `useDailyCall` destructuring | L1403-1425 | `activeSpeakerPeerId`, `flipCamera`, `recoverMediaDevices` |
| `leftNeonAudio` / `rightNeonAudio` / `mediatorNeonAudio` | L1788-1807 | Legacy mort — jamais rendu |
| `<ArenaLayoutManager>` | L3497-3542 | Propagation `activeSpeakerPeerId={activeSpeakerPeerId}` |

### Code legacy mort (TikTokStyleArena)

`leftNeonAudio` / `rightNeonAudio` / `mediatorNeonAudio` (L1788-1807) — calculés, jamais consommés.
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `hooks/usePiP.ts` | 56 | Hook PiP + enableAutoPiP |
| `components/ParticipantVideo.tsx` | 78 | Vidéo + isSmartPiPUI |
| `components/TikTokStyleArena.tsx` | ~4045 | Monolithe arène |
| `hooks/useDailyMeetingEngine.ts` | 482 | Moteur WebRTC + flipCamera |
| `components/Arena/shared/ArenaVideoSurface.tsx` | 252 | Tuile + localControls + pseudoBadge |

## Annexe — SmartPiPManager (destruction D.2, hors liste demandée)

Instancié dans `ArenaLayoutManager.tsx` L178. Source actuelle :
...
## `components/ParticipantVideo.tsx` (77 lignes)
...
## `components/TikTokStyleArena.tsx` (4386 lignes)
...
## `hooks/useDailyMeetingEngine.ts` (481 lignes)
...
## `components/Arena/shared/ArenaVideoSurface.tsx` (251 lignes)

# Rapport d'audit — Phase A.0 (Store volatil + Pipeline Arène pré-Phase A)

**Date d'extraction :** 2026-07-18  
**Commit de référence :** `8e6d510 feat(economy): deflationary gift grid and Lingots wallet semantics`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse diagnostic Phase A.0

### A. Store volatil (`arenaVolatileStore.ts`)

| Élément | État actuel |
|---------|-------------|
| `VisibleMessage` | `id`, `user_name`, `content`, `timestamp`, `initial`, `type?: 'text' \| 'gift'` |
| Champs **absents** (cibles Phase A) | `giftSender`, `giftRecipient`, `giftTemplate` |
| `addMessage` | Spread `{ ...msg, timestamp }` — prêt à accepter nouveaux champs optionnels |
| Cap mémoire chat | 80 messages (`slice(-80)`) |

### B. Pipeline Arène (`TikTokStyleArena.tsx`) — Correctifs Z.2 validés

| Zone | Ligne | État |
|------|-------|------|
| `addRemoteMessage` | L.642–657 | 5 params : `(name, content, initial?, dbId?, type?: 'text' \| 'gift')` → `addMessage({ …, type })` |
| Envoi cadeau `addRemoteMessage` | L.3689 | `'gift'` forcé |
| Envoi cadeau `broadcastMessage` | L.3690–3696 | `type: 'gift'` présent |
| `onMessageReceived` | L.2774–2776 | 6 params → relais `type` vers `addRemoteMessage` |
| Punchline cadeau chat | L.3683–3685 | `.replace('{sender}')` / `.replace('{recipient}')` inline (pas encore dans le store) |

**Verdict :** pipeline `type: 'gift'` **câblé**. Métadonnées structurées cadeau (`giftSender`, etc.) **non persistées** dans `VisibleMessage` — prêt pour Phase A.

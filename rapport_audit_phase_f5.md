# Rapport d'audit — Phase F.5 (Édition Beef + crop teaser)

**Date d'extraction :** 2026-07-21  
**Commit de référence :** `6961f74`  
**Contrainte :** Zéro modification du code source — extraction brute uniquement.

---

## Synthèse architecturale — flux édition teaser

### Cartographie fichiers

| Fichier | Rôle |
|---------|------|
| `components/BeefCard.tsx` | Menu contextuel « Modifier » — prop `onEdit` conditionnelle |
| `app/feed/page.tsx` | État `editBeefId`, wiring `onEdit` → `setEditBeefId`, rendu `<EditBeefModal>` |
| `components/EditBeefModal.tsx` | **Modal d'édition** — chargement beef existant, sélection/remplacement teaser, upload Supabase inline dans `handleSave` |
| `utils/cropImage.ts` | Utilitaire canvas `getCroppedImg` — déjà utilisé par `CreateBeefForm` (Phase F.3) |
| `components/CreateBeefForm.tsx` | **Référence crop F.3** — `react-easy-crop` + overlay recadrage images |

### Câblage BeefCard → Feed → EditBeefModal

```
BeefCard (menu ⋮)
  └─ onEdit prop (L.353-364)
        ↓ clic « Modifier » → onEdit()
Feed page (app/feed/page.tsx)
  └─ onEdit={ () => setEditBeefId(beef.id) }  (L.926-929)
        ↓ editBeefId !== null
  └─ <EditBeefModal beefId={editBeefId} onClose onSaved />  (L.1252-1261)
        ↓ onSaved
  └─ setEditBeefId(null) + loadBeefs() + toast success
```

**Conditions d'affichage du bouton Modifier :**

- `BeefCard` : rendu si `onEdit` est défini (prop optionnelle).
- `Feed` : `onEdit` fourni uniquement si `beef.status === 'pending'` **et** `user?.id === beef.created_by`.

**Snippet BeefCard — menu Modifier :**
...
**Snippet Feed — rendu EditBeefModal :**
...
### Chaîne de données teaser (EditBeefModal)

```
loadData (L.132-244)
  └─ SELECT beefs.video_url, thumbnail
  └─ setTeaserPreview(vUrl || tUrl) — URL distante Supabase
  └─ baselineMediaRef = { video, thumb } — snapshot média initial

<input id="edit-beef-teaser-upload" accept="video/*,image/*" />
        ↓ onChange
handleFileChange (L.250-271)
        ↓ fichier sélectionné
setTeaserFile(file) + URL.createObjectURL → teaserPreview (blob local)
        ↓ reset input (absent — pas de e.target.value = '')
handleSave (L.441-584)
        ↓ si teaserFile !== null
supabase.storage.from('teasers').upload(fileName, teaserFile)  (L.472-486)
        ↓
getPublicUrl → video_url (vidéo) | thumbnail (image)
        ↓
supabase.from('beefs').update({ ..., video_url, thumbnail })  (L.489-500)
```

**Différence clé vs création :** pas de `submitNewBeef` — upload et update inline dans `handleSave`. Si `teaserFile` est `null`, les colonnes média ne sont **pas** touchées (spread conditionnel L.497).

### État React teaser (EditBeefModal)

| State / Ref | Type | Usage |
|-------------|------|-------|
| `teaserFile` | `File \| null` | Fichier local nouvellement choisi — **cible crop F.5** ; `null` = conserver média distant |
| `teaserPreview` | `string \| null` | URL distante (load) ou blob URL (fichier local) pour aperçu UI |
| `remotePreviewIsVideo` | `boolean` | Distingue `<video>` vs `<img>` quand preview = URL distante sans `teaserFile` |
| `teaserPreviewUrlRef` | `Ref<string \| null>` | Revoke blob URL au cleanup / remplacement |
| `baselineMediaRef` | `Ref<{ video, thumb }>` | Restaure preview distante si input vidé (L.258-260) |

**États crop absents (à répliquer depuis F.3) :** `isCropping`, `rawImageUrl`, `crop`, `zoom`, `croppedAreaPixels`, `isProcessingCrop`.

### Comparaison vs CreateBeefForm (crop F.3)

| Aspect | CreateBeefForm (F.3 implémenté) | EditBeefModal (état actuel F.5) |
|--------|--------------------------------|-----------------------------------|
| Import crop | `react-easy-crop`, `getCroppedImg` | **Aucun** |
| Images | `handleFileChange` → `setRawImageUrl` + `setIsCropping(true)` | Upload direct → `setTeaserFile` + preview blob |
| Vidéos | Pipeline direct (pas de crop) | Pipeline direct (identique) |
| Ratio crop UI | `aspect={3 / 4}` dans Cropper | Zone preview `aspect-video` (16:9) — **incohérence visuelle pré-crop** |
| Preview image | `object-contain bg-black` post-crop | `object-contain bg-black` |
| Upload | Délégué à `submitNewBeef` | Inline `handleSave` → bucket `teasers` |
| Média existant | N/A (création) | `baselineMediaRef` + preview distante — **cas spécifique édition** |
| Reset fichier | `setTeaserPreview(null)` | Restaure `baselineMediaRef` (video \|\| thumb) |

### Points d'injection crop F.5 (réplication F.3 → EditBeefModal)

| Zone | Ligne(s) | Action recommandée |
|------|----------|-------------------|
| Imports | L.1-22 | Ajouter `Cropper`, CSS, `getCroppedImg` |
| États crop | après L.121 | Copier bloc F.3 : `isCropping`, `rawImageUrl`, `crop`, `zoom`, `croppedAreaPixels`, `isProcessingCrop` |
| `useEffect` cleanup | L.123-130 | Étendre revoke `rawImageUrl` (pattern CreateBeefForm L.103-113) |
| `handleFileChange` | L.250-271 | Intercepter `image/*` → crop modal ; vidéos inchangées ; reset input `e.target.value = ''` |
| `handleCropSave` | nouveau | `getCroppedImg` → `setTeaserFile(croppedFile)` + blob preview |
| Overlay crop UI | dans `Dialog.Content` | Modal absolue `z-[10005]` — calquer CreateBeefForm L.916-965 |
| `handleSave` upload | L.472-487 | **Inchangé** si `teaserFile` = fichier déjà croppé |
| Annuler crop | — | Restaurer preview via `baselineMediaRef` (pas `null` comme création) |
| Feed / BeefCard | — | **Aucune modification** — crop encapsulé dans `EditBeefModal` |

---

## 1. Code source intégral — `components/EditBeefModal.tsx`
...
## 2. Code source intégral — `utils/cropImage.ts`
...
## 3. Verdict Architecte — réplication crop F.5

| Décision | Recommandation |
|----------|----------------|
| **Scope unique** | Modifier **uniquement** `EditBeefModal.tsx` — Feed et BeefCard restent inchangés |
| **Pattern source** | Copier le bloc crop de `CreateBeefForm` (F.3) : états, `handleCropSave`, overlay `Cropper` |
| **Point d'entrée** | `handleFileChange` L.250-271 — branche `image/*` → crop ; vidéos = pipeline actuel |
| **Ratio crop** | Harmoniser avec F.3 : `aspect={3 / 4}` (CreateBeefForm) ; ajuster zone preview edit si besoin |
| **Output** | `getCroppedImg` → remplace `teaserFile` ; `handleSave` L.472-487 **inchangé** |
| **Média existant** | Annulation crop → restaurer via `baselineMediaRef` (pas `null`) |
| **Upload bucket** | `'teasers'` — même convention `${user.id}_${Date.now()}.${ext}` |
| **Update conditionnel** | Conserver spread `...(teaserFile ? { video_url, thumbnail } : {})` — pas d'écrasement si pas de nouveau fichier |
| **Cleanup mémoire** | Revoke `rawImageUrl` + `teaserPreviewUrlRef` dans `useEffect` unmount |
| **Régression** | Tester : édition sans toucher teaser ; remplacement image croppée ; remplacement vidéo ; annulation crop |
| **Hors scope v1** | Crop vidéo ; suppression explicite du teaser (pas de bouton « retirer » actuellement) |

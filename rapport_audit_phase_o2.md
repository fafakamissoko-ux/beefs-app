# Rapport d'audit — Phase O.2 (Câblage Wallet Store + GIFT_CATALOG)
...
## Observations Architecte — zones à câbler en O.2

| Zone | Lignes | État actuel | Cible O.2 |
|------|--------|-------------|-----------|
| État solde local | 350 | `useState(0)` → `userPoints` | `useWalletStore((s) => s.balance)` |
| Fetch initial points | 1321–1328 | `supabase.from('users').select('points')` + `setUserPoints` | `useWalletStore.initialize(userId)` |
| Recharge popup Stripe | 741–761 | `goBuyPoints()` polling 3s + `setUserPoints` | Realtime wallet store (plus de polling manuel) |
| Affichage solde desktop menu | 3286–3296 | `userPoints` Lingots | `balance` depuis store |
| Affichage solde mobile menu | 4075–4087 | idem | idem |
| Catalogue cadeaux inline | 3722–3734 | Array hardcodé 12 items | `GIFT_CATALOG` depuis `lib/constants/gifts.ts` |
| Envoi cadeau (inline handler) | 3739–3811 | Vérif solde + `fetch /api/gifts/send` + `setUserPoints` | `optimisticDebit(cost)` avant RPC |
| Destinataires cadeaux | 1685–1713 | `giftRecipients` useMemo | **Conserver** (logique métier arène) |
| Props `points` | 140, 221 | Prop optionnelle (non utilisée pour solde UI) | Peut rester ou être retirée plus tard |

**Note :** Il n'existe pas de fonction `handleSendGift` nommée — la logique d'envoi est **inline** dans le `onClick` de chaque bouton cadeau (l.3739–3811).

---

## Code source intégral — `components/TikTokStyleArena.tsx`
  ...
  const goBuyPoints = useCallback(() => {
    ...
    const pollTimer = setInterval(async () => {
      ...
      if (data && data.points > initialPoints) {
        ...
        toast('Lingots crÃ©ditÃ©s !', 'success');
        clearInterval(pollTimer);
            ...
            {showGiftPicker && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {[
                    { emoji: 'ðŸ§‚', label: 'Sel', id: 'salt', cost: 1 },
                    { emoji: 'ðŸŽ¤', label: 'Mic Drop', id: 'mic_drop', cost: 5 },
                    { emoji: 'ðŸŒ¶ï¸', label: 'Spicy', id: 'spicy', cost: 10 },
                    { emoji: 'ðŸ§ ', label: 'Big Brain', id: 'big_brain', cost: 25 },
                    { emoji: 'âš¡', label: 'Foudre', id: 'lightning', cost: 50 },
                    { emoji: 'ðŸ¥Š', label: 'K.O.', id: 'ko', cost: 99 },
                    { emoji: 'ðŸ’£', label: 'Banger', id: 'banger', cost: 199 },
                    { emoji: 'ðŸº', label: 'Loup', id: 'wolf', cost: 500 },
                    { emoji: 'â˜„ï¸', label: 'MÃ©tÃ©ore', id: 'meteor', cost: 1000 },
                    { emoji: 'ðŸŒ‹', label: 'Ã‰ruption', id: 'volcano', cost: 2500 },
                    { emoji: 'ðŸ†', label: 'Champion', id: 'champion', cost: 5000 },
                    { emoji: 'ðŸ', label: 'G.O.A.T', id: 'goat', cost: 10000 },
                  ].map((gift) => (
                    <button
                      key={gift.label}
                      type="button"
                      onClick={async () => {
                        if (userPoints < gift.cost) {
                          toast(`Lingots insuffisants â€” il te manque ${gift.cost - userPoints} Lingots (solde ${userPoints})`, 'error', {
...
*Fin du rapport — extraction Phase O.2 (zéro modification).*
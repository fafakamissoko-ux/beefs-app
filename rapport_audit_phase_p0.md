# Rapport d'audit — Phase P.0 (Anti-Spam Toasts)

**Date d'extraction :** 2026-07-03  
**Objectif :** Analyser `useToast` pour un correctif anti-spam (paramètre d'unicité / ID).  
...
| Élément | État actuel |
|---------|-------------|
| **Fichier source** | `components/Toast.tsx` (94 lignes) — seul export `useToast` du projet |
| **Alternative** | Pas de `hooks/use-toast.ts` ni `components/Toast/index.tsx` |
| **Signature publique** | `toast(message: string, type?: ToastType, options?: ToastOptions)` |
| **Types** | `'success' \| 'error' \| 'info'` |
| **Options exposées** | `action`, `durationMs`, `tone` — **pas de `id` / `dedupeKey`** |
| **Moteur sous-jacent** | [Sonner](https://sonner.emilkowal.ski/) via `sonnerToast.custom()` |
| **Queue** | `<Toaster visibleToasts={3} />` — max 3 toasts visibles, pas de dédup native côté wrapper |
| **ID Sonner interne** | `sonnerToast.custom((id) => …)` reçoit un `id` auto-généré pour `dismiss`, **non configurable depuis l'API publique** |

### Gap P.0 identifié

Pour l'anti-spam, Sonner supporte nativement `id` dans les options du 2ᵉ argument de `toast()` / `toast.custom()` :

```typescript
sonnerToast.custom(renderFn, { id: 'my-unique-key', duration: 4000 });
```

Un toast avec le même `id` **remplace** le précédent au lieu d'en empiler un nouveau.

**Le wrapper actuel ne propage pas cette option** — il faudra étendre `ToastOptions` avec un champ `id?: string | number` et le passer à `sonnerToast.custom(..., { duration, id })`.

---

## Code source intégral — `components/Toast.tsx`
...
## Référence d'usage actuel (consommateurs)

Pattern standard dans le codebase (~20+ fichiers) :

```tsx
const { toast } = useToast();
toast('Message', 'success');
toast('Erreur', 'error', { action: { label: 'Recharger', onClick: () => … } });
toast('Info tour de parole', 'info', { tone: 'ember', durationMs: 6000 });
```

---

## Piste d'implémentation P.0 (observation, hors scope)

```typescript
// ToastOptions — extension proposée
id?: string | number;

// ToastProvider — propagation
sonnerToast.custom(renderFn, { duration: durationMs, id: options?.id });
```

Exemple anti-spam cadeau :

```typescript
toast('Lingots insuffisants', 'error', { id: 'gift-insufficient-balance' });
```

---

*Fin du rapport — extraction Phase P.0 (zéro modification).*

# Rapport d'audit — Phase P.2 (HUD Lingots persistant)

**Date d'extraction :** 2026-07-03  
**Commit de référence :** post 9810d25 (O.2 + P.0 toast dedup)  
...
## 1. Vérification d'intégrité post-déploiement (O.1 / O.2 / P.0)

| Check | Statut | Référence |
|-------|--------|-----------|
| Import `useWalletStore` | OK | l.91 |
| Import `GIFT_CATALOG` | OK | l.92 |
| État legacy `userPoints` | **Absent** | supprimé |
| Store `walletBalance` | OK | l.352 |
| Store `walletInit` | OK | l.353 |
| Store `optimisticDebit` | OK | l.354 |
| Init Realtime WebSocket | OK | l.1311–1314 `walletInit(userId)` |
| Recharge Stripe sans polling | OK | l.745–751 `goBuyPoints` |
| Catalogue `GIFT_CATALOG` | OK | l.3708 |
| Débit optimiste + rollback `sync()` | OK | l.3713, l.3725, l.3781 |
| Toast dedup `id: 'insufficient-funds'` | OK | l.3715–3717 |
| Affichage solde (menus only) | OK | l.3277 (desktop menu), l.4055 (mobile drawer) |

**Conclusion :** Le câblage Zustand est **complet et cohérent**. Le solde n'est visible que dans les menus latéraux/drawer — **pas de HUD persistant** sur la vidéo.

---

## 2. Zone d'ancrage optimale — Pilule monétaire HUD

### Emplacement recommandé : **INDICATEURS SYSTÈME (Haut Droite)**

**Bloc JSX :** l.3364–3410 — commentaire `{/* INDICATEURS SYSTÈME DISCRETS (Haut Droite) */}`
...
| Critère | Détail |
|---------|--------|
| **Visibilité** | Toujours visible en mode non-cinématique, au-dessus de la vidéo (`z-[500]`) |
| **Cohérence UI** | Même style `rounded-full border border-white/10 bg-slate-900/40 backdrop-blur-sm` que Live / Viewers |
| **Safe area** | `top-[max(0.75rem,env(safe-area-inset-top))]` — compatible encoche mobile |
| **Donnée prête** | `walletBalance` déjà souscrit au store — aucun fetch supplémentaire |
| **Interaction** | Parent `pointer-events-none` — la pilule doit avoir `pointer-events-auto` + `onClick={goBuyPoints}` |

### Position suggérée dans le flex

Insérer **après le badge Live, avant le bouton Viewers** (ou entre Viewers et Messages) :

```tsx
<button type="button" onClick={goBuyPoints} className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-white/10 bg-slate-900/40 px-3 py-1.5 ...">
  <Flame className="h-3 w-3 text-cyan-400" />
  <span className="font-mono text-[10px] font-bold text-white">{walletBalance}</span>
</button>
```

**Desktop sidebar** (l.3268+) affiche déjà le solde dans le menu hamburger — le HUD mobile/tablette comble le gap `lg:hidden` sur le bandeau haut-droite.

### Masquage cinématique

Le bloc entier est sous `!isCinematicMode` — le HUD disparaît automatiquement en mode Ciné (comportement souhaité).

---

## 3. Code source intégral — `components/TikTokStyleArena.tsx`
...
*Fin du rapport — extraction Phase P.2 (zéro modification).*
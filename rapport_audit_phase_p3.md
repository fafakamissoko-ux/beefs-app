# Rapport d'audit — Phase P.3 (Refonte UI monétaire)

**Date d'extraction :** 2026-07-04  
**Commit de référence :** `f30fa20` — Phase P.2 (HUD Lingots + IngotIcon)  
**Fichier source :** `components/TikTokStyleArena.tsx`  
**Lignes :** 3908  
**Contrainte :** Zéro modification du code source.

---

## 1. Cartographie des zones monétaires (post P.2)

| Zone | Lignes | Rôle | Donnée affichée |
|------|--------|------|-----------------|
| **`IngotIcon`** (composant) | 115–126 | SVG lingot 3 faces (yellow-300/500/600) | — |
| **HUD persistant** | 3376–3391 | Pilule haut-droite, avant badge Live | `walletBalance` (store Zustand) |
| **Menu Desktop** | 3283–3295 | Dropdown `showArenaMenu` (sidebar header) | `walletBalance` + bouton Recharger |
| **Menu Mobile** | 4075–4087 | Drawer `showArenaMenu` (`lg:hidden`) | `walletBalance` + bouton Recharger |
| **Modale Profil** | 3901–3907 | Stats utilisateur (self only) | `selectedProfile.stats.points` (DB, pas store) |

### Ordre flex — INDICATEURS SYSTÈME (l.3376–3435)

1. **HUD Portefeuille** (`userId &&`) — `goBuyPoints`
2. Badge **Live**
3. Compteur **Viewers** (Eye)
4. **Messages** (MessageCircle + badge unread)
5. **Share** (`sm:flex`)
6. **Menu** mobile (`lg:hidden`)

**Conteneur :** `pointer-events-none` — seuls les boutons ont `pointer-events-auto`.

**Masquage :** bloc entier sous `!isCinematicMode`.

---

## 2. Composant `IngotIcon` (extrait l.115–126)
...
**Usages actuels (4 occurrences JSX) :**

| # | Ligne | Contexte | Classes |
|---|-------|----------|---------|
| 1 | 3388 | HUD haut-droite | `h-3.5 w-3.5 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]` |
| 2 | 3288 | Menu desktop Mon Solde | `h-4 w-4 drop-shadow-md` |
| 3 | 3903 | Modale profil (self) | `h-5 w-5 shrink-0 drop-shadow-md` |
| 4 | 4080 | Menu mobile Mon Solde | `h-5 w-5 drop-shadow-md` |

---

## 3. HUD Portefeuille — extrait (l.3376–3391)
...
{!isCinematicMode && (
  <div className="pointer-events-none absolute right-3 top-[max(0.75rem,env(safe-area-inset-top))] z-[500] flex items-center gap-2 sm:right-4 sm:top-4">
    {userId && (
      <button type="button" onClick={goBuyPoints} className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-white/10 bg-slate-900/40 px-3 py-1.5 shadow-lg backdrop-blur-sm transition-all hover:bg-slate-900/60 active:scale-95" title="Recharger des Lingots">
    ...
    {/* ... Live, Viewers, Messages, Share, Menu ... */}
  </div>
)}
```
...
## 4. Points d'attention pour refonte P.3

- **Duplication solde :** HUD + menus desktop/mobile affichent tous `walletBalance` — purge possible des blocs `Mon Solde` si le HUD suffit.
- **Incohérence profil :** modale utilise `selectedProfile.stats.points` (snapshot DB) vs `walletBalance` (Realtime) — à unifier.
- **`IngotIcon` inline :** défini dans le fichier arène — candidat extraction vers `components/icons/IngotIcon.tsx`.
- **`Flame` lucide :** entièrement retiré (post P.2).
- **Store :** `walletBalance` / `walletInit` / `optimisticDebit` — l.352–354, init l.1311–1314.

---

## 5. Code source intégral — `components/TikTokStyleArena.tsx`
...
function IngotIcon({ className = 'h-4 w-4' }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className={className}>
      {/* Face SupÃ©rieure (LumiÃ¨re) */}
      <polygon points="7,6 19,6 22,12 10,12" className="fill-yellow-300" />
      {/* Face Avant (Ombre moyenne) */}
      <polygon points="10,12 22,12 20,18 8,18" className="fill-yellow-500" />
      {/* Face Gauche (Ombre foncÃ©e) */}
      <polygon points="7,6 10,12 8,18 5,12" className="fill-yellow-600" />
    </svg>
  );
}
        ...
        {!isCinematicMode && (
          <div className="pointer-events-none absolute right-3 top-[max(0.75rem,env(safe-area-inset-top))] z-[500] flex items-center gap-2 sm:right-4 sm:top-4">
            ...
            >
              ...
              <span className="font-mono text-[10px] font-bold text-white">{liveViewerCount > 0 ? liveViewerCount : 'â€”'}</span>
            </button>
            <button
              type="button"
              onClick={() => openDrawer()}
...
*Fin du rapport — extraction Phase P.3 (zéro modification).*
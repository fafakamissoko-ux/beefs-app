# Rapport d'audit — Phase Q.1 (Ergonomie arène)

**Date d'extraction :** 2026-07-04  
**Commit de référence :** `a992906` (post P.3)  
...
### 1. Pilules pseudo — `ArenaVideoSurface.tsx`

| Élément | Ligne | Classes actuelles | Cible Q.1 |
|---------|-------|-------------------|-----------|
| Bouton pseudo | 131 | `w-[65px] sm:w-[120px]` (largeur fixe) | Remplacer par `max-w-[...]` |
| Conteneur pseudo | 117 | `max-w-full` sur pill externe | OK |
| Marquee 5/6 joueurs | 134 | `animate-marquee-pseudo` si `tileCount >= 5 && name.length > 8` | Conserver |
| Wrapper constellation | 219 | `max-w-[200px]` | Peut contraindre la pill |

**Note :** Pas de balise `<video>` native dans ce fichier — délégation à `ParticipantVideo` (l.175–179) avec `enableAutoPiP={isActiveSpeaker || tile.isLocal}`.

### 2. Picture-in-Picture (overlap haut-droite)

Le bouton PiP **n'est pas** dans `TikTokStyleArena` mais dans `components/ParticipantVideo.tsx` :

- `<video disablePictureInPicture={false}>` (l.57–64)
- Bouton `PictureInPicture2` : `absolute top-2 right-2 z-20 hidden md:flex` (l.65–73)
- **Visible uniquement si** `isPiPSupported && !isPiPActive && !enableAutoPiP`
- Sur tuiles **active speaker** ou **local**, `enableAutoPiP=true` → bouton masqué ; auto-PiP activé via `usePiP`

**Collision possible :** bouton PiP en coin haut-droite **de chaque tuile vidéo** (desktop `md+`), pas du HUD global.

### 3. Timer menu — `TikTokStyleArena.tsx`

```tsx
// l.569–573 — À SUPPRIMER pour Q.1
useEffect(() => {
...
**Comparaison :** soundboard garde son timer 3s (l.564–567) — menu arène seul concerné.

### 4. Éléments `absolute right-*` haut de l'écran (collisions)

| Ligne | Composant | Position | z-index |
|-------|-----------|----------|---------|
| 3391 | INDICATEURS SYSTÈME (Live/Viewers/Menu) | `right-2 sm:right-4`, top dynamique banderole | 500 |
| 3507 | Soundboard host | `right-2 top-1/2` (milieu écran) | 250 |
| 3831 | Modale profil (bouton fermer) | `right-3 top-3` | 10 |

Pas d'autre overlay `absolute right` en haut à droite vidéo hors tuiles `ParticipantVideo`.

---

## Code source intégral — `components/Arena/shared/ArenaVideoSurface.tsx` (230 lignes)
...
## Code source intégral — `components/TikTokStyleArena.tsx` (3890 lignes)
...
## Annexe — `ParticipantVideo.tsx` (balise `<video>` + PiP, non demandé mais lié)
...
*Fin du rapport — extraction Phase Q.1 (zéro modification).*
# Rapport d'audit — Phase R.1 (Moteur PNG GPU + Queue FIFO)

**Date d'extraction :** 2026-07-04  
**Commit de référence :** `34a88fd fix(arena): disable PiP overlap, keep menu open, flex pseudo pills (Phase Q.1)`  
**Contrainte :** Zéro modification du code source.

---

## 1. Inventaire `public/gifts/`

### Fichiers présents (12 PNG — catalogue complet)

| Fichier disque | ID catalogue | Attendu R.1 | Taille | Statut nommage |
|----------------|--------------|-------------|--------|----------------|
| `salt.png.png` | salt | `salt.png` | ~992 KB | ⚠️ double extension |
| `mic_drop.png.png` | mic_drop | `mic_drop.png` | ~343 KB | ⚠️ double extension |
| `spicy.png.png` | spicy | `spicy.png` | ~4,9 MB | ⚠️ double extension |
| `big_brain.png.png` | big_brain | `big_brain.png` | ~14,4 MB | ⚠️ double extension |
| `lightning.png.png` | lightning | `lightning.png` | ~6,6 MB | ⚠️ double extension |
| `ko.png.png` | ko | `ko.png` | ~335 KB | ⚠️ double extension |
| `banger.png.png` | banger | `banger.png` | ~4,2 MB | ⚠️ double extension |
| `wolf.png.png` | wolf | `wolf.png` | ~22,3 MB | ⚠️ double extension |
| `meteor.png.png` | meteor | `meteor.png` | ~4,8 MB | ⚠️ double extension |
| `volcano.png.png` | volcano | `volcano.png` | ~9,8 MB | ⚠️ double extension |
| `champion.png.png` | champion | `champion.png` | ~2,5 MB | ⚠️ double extension |
| `goat.png.png` | goat | `goat.png` | ~6,1 MB | ⚠️ double extension |

**Total :** 12/12 IDs `GIFT_CATALOG` couverts — **aucun fichier manquant**.

**Action R.1 requise :** renommer `{id}.png.png` → `{id}.png` **ou** résoudre via helper `/gifts/${id}.png.png`.

**Poids cumulé :** ~81 MB — prévoir lazy-load / preload sélectif tier ≥ 500 Lingots.

---

## 2. Points d'ancrage arène — remplacement Store Volatil

| Zone | Lignes | Rôle actuel | Cible R.1 |
|------|--------|-------------|-----------|
| État local | 963 | `localArenaBigGift` useState | Supprimer → `enqueueBigGift` store |
| Timer reset 6s | 965–968 | `setLocalArenaBigGift(null)` | Supprimer (queue gère lifecycle) |
| Envoi cadeau | 3771–3780 | `setLocalArenaBigGift` + broadcast | `useArenaVolatileStore.getState().enqueueBigGift(...)` |
| Realtime callback | 2799–2800 | `onArenaBigGift` → setState | `enqueueBigGift` depuis store |
| Portal rendu | 4196 | `<FullscreenGiftAnimation localBigGift={...} />` | Consommer queue store directement |

---

## 3. Code source intégral — `lib/stores/arenaVolatileStore.ts`
...
export const useArenaVolatileStore = create<ArenaVolatileStore>((set) => ({
  ...
  addMessage: (msg) => set((state) => {
    // DÃ©duplication stricte pour Ã©viter les fantÃ´mes
    if (state.messages.some(m => m.id === msg.id)) return state;
    // CapacitÃ© maximale de 80 messages en mÃ©moire pour prÃ©server la RAM
  ...
  addReaction: (reaction) => set((state) => {
    ...
    // Limite stricte de 40 particules simultanÃ©es Ã  l'Ã©cran pour Ã©viter la surcharge GPU
...
## 4. Code source intégral — `components/Arena/FullscreenGiftAnimation.tsx`
    ...
    return (
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="max-w-[min(90vw,24rem)] text-sm text-white/85">
          <span className="font-bold text-amber-100">{senderName}</span> â€” {label} Â· {cost} pts
        </p>
      </div>
    );
  }
...
type Props = {
  /** ConservÃ© pour lâ€™API parente ; le flux rÃ©seau est gÃ©rÃ© par `useArenaRealtime` + `localBigGift`. */
  roomId: string;
  /** Cadeau Â« gros budget Â» Ã  animer (Ã©metteur inclus : `self: false` cÃ´tÃ© hook). */
...
## 5. Code source intégral — `components/TikTokStyleArena.tsx` (3885 lignes)
...
/** DurÃ©e par dÃ©faut au lancement Â« Lancer le beef Â» (rÃ©gie : +/- au-delÃ ). */
const DEFAULT_BEEF_DURATION = 60 * 60; // 60 min
/** Plafond ajustable depuis la rÃ©gie (prolongations). */
...
interface TikTokStyleArenaProps {
  ...
  userId: string;
  userName: string;
  /** Toujours fourni par la page arÃ¨ne (pas de dÃ©faut Â« viewer Â» â€” Ã©vite des GET /api/beef/access fantÃ´mes). */
  ...
  /** Jeton Daily `GET /api/beef/access` (mÃ©diateur / challenger / avant fetch spectateur). */
...
// ðŸ”¥ TOP 10 RÃ‰ACTIONS (affichÃ©es par dÃ©faut)
const TOP_10_REACTIONS = [
  'ðŸ‘', 'ðŸ˜‚', 'ðŸ”¥', 'ðŸ’¯', 'ðŸ‘', 'ðŸ˜®', 'ðŸ’€', 'â¤ï¸', 'ðŸŽ‰', 'ðŸš€'
];

// ðŸ”¥ TOUTES LES RÃ‰ACTIONS POPULAIRES (80)
const POPULAR_REACTIONS = [
  'ðŸ‘', 'ðŸ‘Ž', 'ðŸ˜‚', 'ðŸ”¥', 'ðŸ’¯', 'ðŸ‘', 'ðŸ¤”', 'ðŸ˜®', 'ðŸ’€', 'ðŸŽ¯',
  'âš¡', 'ðŸ’ª', 'ðŸ§ ', 'ðŸ‘€', 'ðŸ¤¯', 'ðŸ˜¡', 'â¤ï¸', 'ðŸŽ‰', 'ðŸ™Œ', 'ðŸ’Ž',
  'ðŸŒŸ', 'âœ¨', 'ðŸš€', 'ðŸ’¥', 'ðŸ¤¡', 'ðŸ‘½', 'ðŸ‘»', 'ðŸ¥¶', 'ðŸ¥µ', 'ðŸ˜Ž',
  'ðŸ¤“', 'ðŸ¥³', 'ðŸ¤¬', 'ðŸ¤®', 'ðŸ¤¢', 'ðŸ¤§', 'ðŸ˜‡', 'ðŸ¤«', 'ðŸ¤­', 'ðŸ¥±',
  'ðŸ¤Œ', 'ðŸ«¶', 'ðŸ¤', 'ðŸ¤˜', 'ðŸ¤™', 'ðŸ–ï¸', 'ðŸ‘Š', 'ðŸ™', 'ðŸ†', 'ðŸ¥‡',
  'ðŸ—£ï¸', 'ðŸŽ™ï¸', 'ðŸŽ¤', 'ðŸŽ§', 'ðŸ“»', 'ðŸŽ¸', 'ðŸ¥', 'ðŸŽ­', 'ðŸŽ¨', 'ðŸŽ¬',
  'ðŸ¿', 'ðŸ”', 'ðŸ•', 'ðŸ»', 'ðŸ¥‚', 'ðŸ¾', 'ðŸ§Š', 'ðŸ§‚', 'ðŸŒ¶ï¸', 'ðŸ¥©',
  'ðŸ›‘', 'ðŸš§', 'ðŸš¨', 'ðŸ§¯', 'ðŸ¥Š', 'ðŸ¥‹', 'ðŸ¤º', 'ðŸ‹ï¸', 'ðŸ¤¸', 'âœ…'
];

/** Bandeau mobile : 10 emojis scroll ; desktop : grille 2Ã—5 + panneau ðŸ˜€ pour le reste. */

const HEART_ON_FIRE = 'â¤ï¸â€ðŸ”¥';
...
const PICKER_REACTIONS = POPULAR_REACTIONS.filter((e) => {
  ...
  if (e === 'â¤ï¸' || e === HEART_ON_FIRE) return false;
  return true;
});

/** CÅ“ur / pouce : particules sur lâ€™anneau du challenger (pas dâ€™emoji flottant). */
const INTEGRATED_SUPPORT_REACTIONS = new Set<string>(['â¤ï¸', HEART_ON_FIRE, 'ðŸ‘']);
...
export function TikTokStyleArena({
  ...
  // â”€â”€ AUTH HOOK (Conversion des anonymes + mur freemium mandatory) â”€â”€
  ...
  useEffect(() => {
    console.log("=== ðŸ©º HEALTH CHECK SUPABASE REALTIME ===");
  ...
  /** MediaStream du prÃ©-joint (mÃ©diateur / challenger) â€” rÃ©utilisÃ© par Daily pour Ã©viter un 2áµ‰ getUserMedia bloquÃ© sur mobile. */
  ...
  /** Spectateur promu co-hÃ´te : le mÃ©diateur a acceptÃ© lâ€™invitation (beef_participants). */
  const [acceptedInviteAlert, setAcceptedInviteAlert] = useState(false);

  // â”€â”€ AURA "FERVEUR SOCIALE" â”€â”€
  ...
  /** Heat Index global : activitÃ© de la salle (chat, spectateurs, rÃ©actions) â€” lueur chaude sur le bandeau vidÃ©o. */
  ...
  /** True dÃ¨s quâ€™au moins un challenger attendu a Ã©tÃ© vu dans la room Daily (Ã©vite la fin auto tant quâ€™on attend les connexions). */
  const challengersEverJoinedRef = useRef(false);
  /** Ã‰vite de spammer le toast Â« challengers partis Â» tant que la room reste vide */
  ...
  /** BanniÃ¨re partagÃ©e Â« tour de parole Â» (remplace le toast pour tous les participants) */
  const [floorAnnouncement, setFloorAnnouncement] = useState<{
    name: string;
    slot: ChallengerSlotId;
  } | null>(null);

  /** DÃ©bat structurÃ© (budget challengers, tours imposÃ©s, micros) */
  ...
  /** Quand le mÃ©diateur parle : les chronos challengers sont en pause (ne consomment pas le budget) */
  const [mediatorHoldingFloor, setMediatorHoldingFloor] = useState(false);
  /** Coupure micro imposÃ©e par le mÃ©diateur (broadcast â€” le toggle local seul ne suffisait pas) */
  ...
  useEffect(() => {
    const goOffline = () => { setIsOffline(true); toast('Connexion perdue â€” reconnexion...', 'error'); };
    const goOnline = () => { setIsOffline(false); toast('Connexion rÃ©tablie', 'success'); };
  ...
  const [showAllReactions, setShowAllReactions] = useState(false); // NEW: Toggle pour afficher toutes les rÃ©actions
  /** Portail body : cadeaux / panneau rÃ©actions au-dessus de la vidÃ©o (z-50) */
  ...
  /** Colonne emoji / cadeaux / partage â€” fermeture au tap extÃ©rieur */
  ...
  /** Accumulateur dâ€™Aura (rÃ©actions / tap intÃ©grÃ©s) â€” flush rÃ©seau ~1,5 s pour limiter le flood. */
  const auraBufferRef = useRef<AuraBatchPayload>(createZeroAuraBatch());
  /** Snapshot pour `aura_master_sync` (intervalle sans recrÃ©er la closure). */
  ...
  // Auto-fermeture : 3s soundboard, 3s menu PC (pickers : fermeture au tap extÃ©rieur uniquement)
  useEffect(() => {
  ...
  /** Clic extÃ©rieur â†’ fermer la rÃ©gie (le backdrop gÃ¨re dÃ©jÃ  le tap sur le voile) */
  useEffect(() => {
  ...
  // Soundboard : clic extÃ©rieur du dock â†’ repli
  useEffect(() => {
    if (!soundboardExpanded) return;
  ...
  // Moderator controls â€” mÃªme logique normalisÃ©e que les pages arÃ¨ne / live (UUID casse / espaces).
  ...
  const fetchPendingInvites = useCallback(async () => {
    ...
    if (error || !data) {
      console.warn('[Live] InvitÃ©s en attente : chargement impossible');
      return;
    }
  ...
  const handleAcceptPendingInvite = useCallback(
    async (inviteUserId: string) => {
      ...
      if (!r.ok) return;
      toast('Challenger acceptÃ© !', 'success');
      void fetchPendingInvites();
    },
    [roomId, toast, fetchPendingInvites, runBeefManage],
  );

  /** Refus : UPDATE â†’ declined (pas de DELETE RLS mÃ©diateur sur beef_participants). */
  ...
  const statsRef = useRef({
    ...
    liveViewerCount: 0,
    messagesCount: 0,
    /** RÃ©sonance Â« distante Â» (rÃ©actions reÃ§ues en broadcast), cumulÃ©e cÃ´tÃ© client. */
    votesA: 0,
    votesB: 0,
    votesC: 0,
    votesD: 0,
  ...
  /** Chat temps rÃ©el : dÃ©dup + insert (rÃ©utilisÃ© par `useArenaRealtime`, avant Daily). */
  ...
  /** Boost par tap / rÃ©action soutenue : fixe 15 (Ã©quitable, gÃ©nÃ©reux vs decay âˆ’3 / 500 ms). */
  const getAuraBoost = () => 15;

  const emitTapSupport = useCallback(
    (target: ArenaSupportSlotId) => {
      if (requireAuth('Fais grimper l\'Aura', 'CrÃ©e un compte gratuit pour tapoter l\'Ã©cran et soutenir tes favoris !')) return;
  ...
  // Participant roles from DB â€” maps Daily.co userNames to beef roles
  const [participantRoles, setParticipantRoles] = useState<Record<string, BeefParticipantRowMeta>>({});
  // Ordre canonique (accepted first â†’ is_main â†’ created_at) pour coller au feed
  ...
  const loadParticipants = useCallback(async () => {
    ...
    // --- DÃ‰TECTEUR D'EXPULSION (LIMBO FIX) ---
    if (!isViewer && !isHost && data) {
      const amIStillHere = data.some((p: { user_id: string }) => p.user_id === userId);
      if (!amIStillHere) {
        toast('Vous avez Ã©tÃ© renvoyÃ© dans les gradins par la rÃ©gie.', 'error');
    ...
    // Seuls les "accepted" obtiennent un halo dans la grille gÃ©omÃ©trique
    ...
    // MÃªme ordre que le feed : accepted â†’ is_main â†’ created_at
  ...
  const expectedUids = useMemo(() => {
    // PrioritÃ© Ã  l'ordre canonique DB (accepted first â†’ is_main â†’ created_at)
    ...
    // Fallback si l'Ã©tat n'est pas encore peuplÃ©
  ...
  // Chrono global â€” dÃ©faut 60 min, plafond MAX_BEEF_DURATION Ã  la rÃ©gie
  ...
  /** Rempli aprÃ¨s `useDailyCall` â€” `endBeef` vit au-dessus du hook et ne peut pas fermer sur `leave` en closure directe. */
  const leaveRef = useRef<() => Promise<void>>(async () => {});
  const stopAllMediaTracksRef = useRef<() => void>(() => {});

  /** API broadcast issue du hook (remplie Ã  chaque rendu aprÃ¨s `useArenaRealtime`). */
  ...
  /** SÃ©rialise les INSERT chat pour Ã©viter les rafales concurrentes cÃ´tÃ© RLS. */
  const messageSendChainRef = useRef(Promise.resolve());

  // DÃ©compte partagÃ© (deadline `beefEndsAtMsRef`) â€” mÃ©diateur + clients synchronisÃ©s
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (next <= 0 && isHostRef.current) {
        ...
        endBeefRef.current?.('Temps Ã©coulÃ©');
      }
    };
    tick();
  ...
  /** MÃ©morise le panneau Â« prÃ©fÃ©rÃ© Â» pour les rÃ©actions intÃ©grÃ©es (pas de compteur de vote). */
  ...
  /** Le chrono global du beef ne doit pas Ãªtre figÃ© par la micro du mÃ©diateur ou lâ€™Ã©tat audio des challengers. */
  ...
  const handleStartBeef = useCallback(
    async (durationSec: number) => {
      ...
      try {
        ...
        setTimerActive(true);
        setTimerPaused(false);
        toast('Le beef a commencÃ©.', 'success');
        queueMicrotask(() => scheduleBeefGlobalTimerBroadcast());
      } catch (err) {
  ...
  const endBeef = useCallback(async (reason: string = 'TerminÃ© par le Ref') => {
  ...
  const handleMediatorVerdict = useCallback(
    async (kind: 'resolved' | 'closed' | 'rematch') => {
      ...
      if (kind === 'resolved') {
        ...
        window.setTimeout(() => void endBeef('Verdict : rÃ©solu'), 1600);
        return;
      }
      if (kind === 'closed') {
  ...
  // â”€â”€ Aura decay (âˆ’3 toutes les 500ms) â”€â”€
  ...
  // Auto-Fever mÃ©diateur : 15 s puis reset â€” deps SANS auraFeverMed sinon le cleanup annule le timer dÃ¨s que fever passe Ã  true
  useEffect(() => {
  ...
  /** Aura prestige-gold â€” cadre sponsor : les gains remontent au Host quand un soutien financier est dÃ©tectÃ©.
   *  TODO: brancher sur l'Ã©vÃ©nement gift broadcast ; pour l'instant, activÃ© par l'aura A ou B > 60. */
  ...
  /** Grille challengers : index = slot attribuÃ© par reconcilePeers (A=0 â€¦ F=5). */
  ...
  /** Pas de bulles Â« onboarding Â» quand la salle est dÃ©jÃ  active ou pendant la connexion Daily */
  ...
  // MÃ©diateur : mÃ©moriser quâ€™un challenger invitÃ© est rÃ©ellement entrÃ© dans la room (rÃ©conciliation identitÃ©)
  useEffect(() => {
  ...
  // â”€â”€ AUTO-END: Detect mediator or all challengers leaving â”€â”€
  useEffect(() => {
    ...
    if (!mediatorPresent && !isHost && mediatorWasConnectedRef.current) {
      // MÃ©diateur absent : avertissement + dÃ©compte â€” on ne termine **pas** le beef depuis un client non-hÃ´te
      // (sinon navigation / onglet achat / coupure rÃ©seau court-circuitent le direct Ã  tort).
        ...
        const countdown = setInterval(() => {
          setMediatorGraceSeconds(prev => {
            if (prev <= 1) {
              ...
              toast(
                'Le Ref est toujours absent â€” le direct reste ouvert jusquâ€™Ã  son retour ou la fin cÃ´tÃ© Ref.',
                'info',
              );
              return 0;
            }
      ...
      // Mediator reconnected â€” cancel grace period
    ...
    // Challengers partis, mÃ©diateur toujours prÃ©sent : ne pas terminer le beef â€” notification unique.
    if (
      isHost &&
      ...
      if (!challengersAllLeftNotifiedRef.current) {
        ...
        toast('Les challengers ont quittÃ© la room â€” le direct continue. Tu peux terminer le beef depuis la rÃ©gie.', 'info');
      }
  ...
  // Auto-join quand Â« Rejoindre Â» + URL Daily + jeton (fournis par la page parente).
  useEffect(() => {
  ...
  const handleRaiseHand = useCallback(async () => {
    ...
    try {
      ...
      toast('Demande envoyÃ©e ! Le Ref va te rÃ©pondre.', 'success');
    } catch (error) {
      console.error('Erreur lors de la demande');
      const msg = error instanceof Error ? error.message : 'Impossible dâ€™envoyer la demande.';
      toast(msg, 'error');
    }
  }, [userId, roomId, toast]);

  // Spectateurs uniquement (pas mÃ©diateur ni challengers)
  useEffect(() => {
  ...
  /** Spectateur : LIVE Â« chaud Â» â€” au moins un slot grille 0â€“3 avec flux (y compris orphelins). */
  ...
  const rightPanel = isHost
    ...
    : isViewer
      ? challengerRemoteSlots[1] ?? null
      : challengerRemoteSlots[0] ?? null;
  /** Si le flux local Daily est mappÃ© sur le panneau droit (rare mais possible selon lâ€™ordre des peers). */
  ...
  /** Halos nÃ©on (Phase 2) : parole rÃ©elle Daily + micro ouvert sur la piste audio. */
  const leftNeonAudio =
  ...
  /** Slot affichÃ© sur les panneaux (spectateurs : parfois pas de match arenaUserId â†’ fallback banniÃ¨re). */
  ...
  const handleMediatorChallengerMute = useCallback(
    (sessionId: string, debaterId: string | null, muted: boolean) => {
      ...
      if (debaterId) {
        ...
        /** Couper le micro du locuteur actif = fin du tour de parole (chrono arrÃªtÃ©) */
  ...
  const handleMuteAll = useCallback(() => {
    ...
    toast('Silence imposÃ© Ã  tous', 'info');
  ...
  const handleReaction = (emoji: string) => {
    if (requireAuth('Donne de la force', 'CrÃ©e un compte gratuit pour envoyer des rÃ©actions.')) return;
    onReaction(emoji);

    const integrated = INTEGRATED_SUPPORT_REACTIONS.has(emoji);
    const slotPick: ChallengerSlotId = myVote ?? lastPulseSideRef.current ?? 'A';
    const isHeartEmoji = emoji === 'â¤ï¸' || emoji === HEART_ON_FIRE;
  ...
  const startHotMicTurn = useCallback(
    (slot: ChallengerSlotId, durationSec: number, opts?: { force?: boolean }) => {
      if (speakingTurnActive && !opts?.force) {
        toast('Un tour de parole est dÃ©jÃ  en cours.', 'info');
        return;
      }
      ...
      if (!debaterId || !activePanel?.sessionId) {
        toast('Challenger non connectÃ© pour ce slot.', 'info');
        return;
      }
  ...
  // Compte Ã  rebours du tour (+ budget Â« temps challengers Â» si dÃ©bat structurÃ©, sans grignoter le chrono global du beef)
  useEffect(() => {
    ...
    speakingTurnIntervalRef.current = setInterval(() => {
      ...
      setSpeakingTurnRemaining((prev) => {
        if (prev <= 1) {
          stopTimer();
          if (isHost) {
            toast('Temps de parole Ã©coulÃ© â€” donne la parole au suivant quand tu es prÃªt.', 'info');
          }
          return 0;
        }
        return prev - 1;
  ...
  /** Micro challengers : hot mic (tour actif) mÃªme hors dÃ©bat structurÃ© ; sinon rÃ¨gles structurÃ©es. */
  useEffect(() => {
    ...
    if (speakingTurnActive && speakingTurnTarget) {
      if (!floorHotMic) {
        setLocalAudioEnabled(false); // Coupe forcÃ©e si ce n'est pas son tour
      }
      return; // Si c'est son tour, on quitte l'effet pour le laisser libre de son bouton
    }
    if (!structuredDebateEnabled) {
      return; // En dÃ©bat libre, on quitte l'effet. L'utilisateur gÃ¨re son propre mute.
    }
    setLocalAudioEnabled(false);
  }, [
    isViewer,
  ...
  const inviteDebater = async () => {
    ...
    if (debaters.some(d => d.name === username)) {
      toast('Ce dÃ©batteur est dÃ©jÃ  dans le dÃ©bat', 'info');
      return;
    }
    ...
    setInviteInput('');
    toast(`Invitation envoyÃ©e Ã  ${foundUser.display_name || foundUser.username}`, 'success');
    void fetchPendingInvites();
  };
  ...
  const handleInviteFromModal = async (invitedUserId: string) => {
    ...
    toast('Invitation envoyÃ©e !', 'success');
    void fetchPendingInvites();
  };
  ...
  const toggleFollowProfileTarget = async () => {
    if (requireAuth('Abonne-toi', 'CrÃ©e un compte pour suivre ce profil.')) return;
    ...
    try {
      ...
      toast('Impossible de modifier lâ€™abonnement', 'error');
    }
  };
  ...
  const handleSendMessage = () => {
    if (requireAuth('Rejoins la discussion', 'CrÃ©e un compte gratuit pour envoyer des messages dans le chat.')) return;
    ...
    const attemptInsert = async (attempt: number): Promise<void> => {
      ...
      if (error && isRlsPolicyError(error)) {
        toast(
          'Envoi temporairement refusÃ© (limite ou droits). RÃ©essaie dans un instant.',
          'error',
        );
      } else {
        toast('Impossible dâ€™envoyer le message', 'error');
      }
  ...
  // Join: enregistre le flux prÃ©-acquis puis lance join() via lâ€™effet ci-dessus
  ...
  const handleLeave = useCallback(async () => {
    ...
    if (isHost) {
      const ok = window.confirm(
        'Mettre fin au beef pour tous les participants ? Cette action est dÃ©finitive.',
      );
      if (!ok) return;
    }
    setIsLeaving(true);
  ...
  const arenaRealtimeCallbacks = {
    ...
    onMediationToss: (firstName: string) => {
      toast(`${firstName} parle en premier (tirage au sort dÃ©cidÃ© par le Ref).`, 'success');
    },
    ...
    >
        ...
        <div className="absolute inset-0 z-[8000] bg-black/40 backdrop-blur-sm">
          ...
          {!effectiveDailyRoomUrl && (
            <div className="absolute bottom-10 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/10 bg-slate-900/65 px-4 py-2 text-xs font-semibold text-cyan-400 backdrop-blur-md">
              ...
              PrÃ©paration de la room vidÃ©o...
            </div>
          )}
        </div>
      )}
        ...
        >
          <div className="flex w-max min-w-full animate-marquee-continuous whitespace-nowrap">
            <span className="mx-4 text-[10px] font-black uppercase tracking-widest text-white/90">
              {announcementTicker} â€¢ {announcementTicker} â€¢ {announcementTicker} â€¢ {announcementTicker}
            </span>
            <span className="mx-4 text-[10px] font-black uppercase tracking-widest text-white/90">
              {announcementTicker} â€¢ {announcementTicker} â€¢ {announcementTicker} â€¢ {announcementTicker}
            </span>
      ...
      <AnimatePresence>
        {isCinematicMode && (
          ...
          >
            <X className="h-4 w-4" aria-hidden /> Quitter CinÃ©
          </motion.button>
        )}
      </AnimatePresence>

      {/* Instant black overlay when leaving â€” hides camera before tracks stop */}
      {isLeaving && !beefEnded && (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-gray-500 text-sm">DÃ©connexion...</span>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <h2 id="beef-end-summary-title" className="text-2xl font-bold text-white">Beef terminÃ©</h2>
              <p className="text-sm text-gray-400">{endSummary.endReason}</p>
            </div>

            {/* Stats (pas de compteur de votes : le soutien se lit sur lâ€™aura en direct) */}
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
                <div className="text-2xl font-bold text-cyan-400">{endSummary.duration}</div>
                <div className="mt-1 text-xs text-gray-500">DurÃ©e</div>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
            ...
            <div className="mt-3 rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
              <div className="mb-3 text-center font-mono text-xs uppercase tracking-widest text-gray-400">RÃ©sonance GÃ©nÃ©rÃ©e</div>
            ...
            <div className="space-y-3 pt-2">
              <p className="text-xs text-gray-500 leading-relaxed px-1">
                Il nâ€™y a pas de fil de commentaires sur cet Ã©cran : les spectateurs peuvent{' '}
                <span className="text-gray-400">noter le Ref</span> (Ã©toiles + commentaire) depuis le rÃ©sumÃ© du
                beef.
              </p>
              <motion.button
                type="button"
      ...
      {/* â”€â”€ NETWORK RECONNECTION OVERLAY â”€â”€ */}
      {isOffline && !beefEnded && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <p className="text-gray-400 text-sm mt-1">VÃ©rifie ta connexion internet</p>
        </motion.div>
      )}
      ...
      {!isCinematicMode && (
        ...
        <header className="relative z-30 shrink-0 flex items-center gap-3 border-b border-white/10 pl-2 pr-4 py-3" data-cinema-stay>
          ...
          <button type="button" onClick={() => setShowViewerList(true)} className="flex shrink-0 items-center gap-1 rounded-full px-2 py-1 text-white/80 hover:bg-white/10">
            ...
            <span className="font-mono text-[11px] font-medium">{liveViewerCount > 0 ? liveViewerCount : 'â€”'}</span>
          </button>
          {showArenaMenu && (
            <div className="absolute left-4 top-full z-[200] mt-2 flex w-64 flex-col rounded-2xl border border-white/10 bg-slate-950/75 py-2 backdrop-blur-md shadow-2xl" data-cinema-stay onClick={(e) => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-2 gap-1 p-2">
                <button type="button" onClick={() => { setShowArenaMenu(false); setIsCinematicMode(true); }} className="flex flex-col items-center gap-2 rounded-xl p-3 text-white transition-colors hover:bg-white/10">
                  ...
                  <span className="text-xs font-medium">CinÃ©matique</span>
                </button>
              ...
              <div className="border-t border-white/10 p-2">
                ...
                <button type="button" onClick={() => { setShowArenaMenu(false); window.open('/settings', '_blank'); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-gray-300 transition-colors hover:bg-white/10 hover:text-white">
                  <SettingsIcon className="h-4 w-4" /> ParamÃ¨tres
                </button>
              </div>
        ...
        <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
          ...
          <div id="dock-desktop" className="mt-auto flex w-full shrink-0 items-center gap-2 pl-2 pr-3 py-3 bg-slate-900/40 backdrop-blur-sm border-t border-white/10 shadow-lg">
            ...
            <button onClick={() => { setShowGiftPicker(false); setShowAllReactions(!showAllReactions); }} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/[0.08] bg-white/10 text-white shadow-[0_4px_16px_rgba(0,0,0,0.2),inset_0_1px_1px_rgba(255,255,255,0.2)] transition-transform active:scale-95 disabled:opacity-30">ðŸ˜€</button>
            <button
              type="button"
              onClick={() => {
                if (requireAuth('Offre un cadeau', 'CrÃ©e un compte gratuit pour envoyer des cadeaux Ã©piques et faire briller ton nom !')) return;
                setShowAllReactions(false);
      ...
      {/* === ZONE 2 : LA VIDÃ‰O (AVEC OVERLAY CHAT MOBILE) === */}
      <div className="relative flex h-full min-w-0 flex-1 flex-col overflow-hidden bg-transparent z-10">
        ...
        {/* INDICATEURS SYSTÃˆME DISCRETS (Haut Droite) */}
        {!isCinematicMode && (
          <div className={`pointer-events-none absolute right-2 sm:right-4 z-[500] flex items-center gap-1.5 sm:gap-2 transition-all duration-300 ${arenaHasAnnouncement ? 'top-[max(2.5rem,calc(env(safe-area-inset-top)+1.75rem))]' : 'top-[max(0.5rem,env(safe-area-inset-top))]'}`}>
            ...
            >
              ...
              <span className="font-mono text-[9px] sm:text-[10px] font-bold text-white">{liveViewerCount > 0 ? liveViewerCount : 'â€”'}</span>
            </button>
        ...
        {/* OVERLAY CHAT MOBILE (IntÃ©grÃ© Ã  la vidÃ©o, invisible sur PC) */}
        {!isCinematicMode && (
          <div
            data-cinema-stay
      ...
      {/* SOUNDBOARD (HOST) â€” icÃ´ne Music, pilule horizontale au clic */}
          ...
          <AnimatePresence>
            {soundboardExpanded && (
              ...
              >
                {(
                  [
                    { id: 'horn', emoji: 'ðŸ“¢', label: 'Airhorn' },
                    { id: 'laugh', emoji: 'ðŸ˜‚', label: 'Rires' },
                    { id: 'slap', emoji: 'ðŸ¥Š', label: 'Punch' },
                    { id: 'drumroll', emoji: 'ðŸ¥', label: 'Tension' },
                    { id: 'crickets', emoji: 'ðŸ¦—', label: 'Malaise' },
                    { id: 'bell', emoji: 'ðŸ””', label: 'Ding' },
                  ] as const
                ).map((sfx) => (
                  <button
                    key={sfx.id}
        ...
        <MediatorSidebar
          ...
          onEjectParticipant={async (sid) => {
            ...
            if (ok) {
              ...
              toast('Participant expulsÃ©', 'success');
            } else {
            ...
            {showAllReactions && (
              ...
              >
                <div className="mb-2 flex items-center justify-between gap-2 border-b border-white/[0.08] pb-2">
                  <span className="pl-0.5 text-[11px] font-semibold text-white/75">RÃ©actions</span>
                  <button
                    type="button"
                    onClick={() => setShowAllReactions(false)}
                    aria-label="Fermer le panneau de rÃ©actions"
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!optimisticDebit(gift.cost)) {
                          toast(`Lingots insuffisants â€” il te manque ${gift.cost - walletBalance} Lingots`, 'error', {
                        ...
                        try {
                          ...
                          if (!targetUserId) {
                            toast('Participant non connectÃ©', 'error');
                          ...
                          const msgContent = `a offert ${gift.emoji} ${gift.label} (${gift.cost} Lingots) Ã  ${targetName}`;
                          ...
                          toast(`${gift.emoji} ${gift.label} envoyÃ© !`, 'success');
                        } catch (err: unknown) {
                          useWalletStore.getState().sync();
              ...
              <div className="relative px-6 pb-6 -mt-12">
                ...
                <div className="mb-4 flex flex-wrap justify-center gap-x-6 gap-y-2">
                  ...
                  <div className="text-center">
                    ...
                    <span className="ml-1 text-sm text-gray-400">MÃ©diations</span>
                  </div>
                  <div className="text-center">
                    <span className="text-2xl font-black text-white">{selectedProfile.stats.followers}</span>
                    <span className="ml-1 text-sm text-gray-400">AbonnÃ©s</span>
                  </div>
                  <div className="text-center">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    {userId && selectedProfile.id !== userId && (
                      ...
                      >
                        {profileFollowsTarget ? 'AbonnÃ© âœ“' : 'Suivre'}
                      </button>
                    )}
                    {userId && selectedProfile.id !== userId && (
                      <button
        ...
        {acceptedInviteAlert && !beefEnded && (
          ...
          >
            ...
            >
              ...
              >
                Invitation Ã  la mÃ©diation
              </h2>
              <p className="mb-6 text-sm text-white/60">
                Le Ref souhaite t&apos;entendre. Installe-toi Ã  la table des Ã©changes en prÃ©parant ta camÃ©ra et
                ton micro.
              </p>
              <button
                type="button"
              ...
              <div className="flex flex-col px-4 pb-[max(2rem,env(safe-area-inset-bottom))]">
                ...
                <div className="mb-5 grid grid-cols-4 gap-3">
                  <button type="button" onClick={() => { setShowArenaMenu(false); setIsCinematicMode(true); }} className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold text-white/80">CinÃ©</span>
                  </button>
                ...
                <div className="mb-5 flex flex-col gap-1 rounded-2xl bg-white/5 p-2">
                  ...
                  <button type="button" onClick={() => { setShowArenaMenu(false); window.open('/settings', '_blank'); }} className="flex items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium text-white transition-colors hover:bg-white/10">
                    <SettingsIcon className="h-5 w-5 text-gray-400" /> ParamÃ¨tres
                  </button>
                </div>
...
*Fin du rapport — extraction Phase R.1 (zéro modification).*
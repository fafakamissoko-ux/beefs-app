# Rapport d'audit — Phase S.1 (Seuil cadeaux + Soundboard + Chat mobile)

**Date d'extraction :** 2026-07-05  
**Commit de référence :** `0a9af57 feat(arena): FIFO big-gift queue with PNG fullscreen animation (Phase R.1)`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse des points d'ancrage S.1

| Objectif S.1 | Fichier | Zone | Détail actuel |
|--------------|---------|------|---------------|
| Abaisser le seuil d'animation cadeau | `TikTokStyleArena.tsx` | L.3765 | `if (gift.cost >= 500)` — seul garde-fou réseau/FIFO |
| Vérifier le moteur d'animation | `FullscreenGiftAnimation.tsx` | L.8 | `ANIM_MS = 4000` — durée de vie ; **aucun seuil de coût interne** |
| Supprimer le Soundboard inactif | `TikTokStyleArena.tsx` | L.333, L.562–594, L.3490–3548 | État `soundboardExpanded`, effets auto-fermeture, bloc JSX `Music` |
| Corriger collision chat mobile | `ArenaChatMessages.tsx` | L.47–49, L.58 | Mobile : `w-fit max-w-[70vw]` / `max-w-[70%]` |
| Conteneur overlay mobile | `TikTokStyleArena.tsx` | L.3458–3481 | `absolute inset-x-0 bottom-0 z-[160] max-h-[45dvh]` |

**Note :** Le moteur FIFO (`enqueueBigGift` / `shiftBigGift`) accepte tout payload sans filtrer le coût — la modification du seuil se fait exclusivement côté arène avant l'enqueue.

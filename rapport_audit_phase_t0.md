# Rapport d'audit — Phase T.0 (Assets Next.js + Soundboard + Grille mobile)

**Date d'extraction :** 2026-07-05  
**Commit de référence :** `963e6fb fix(arena): unlock all gift animations, remove soundboard, tighten mobile chat (Phase S.1)`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse des points d'ancrage Phase T

| Objectif T | Fichier | Zone | Détail actuel |
|------------|---------|------|---------------|
| Désactiver optim. Next.js sur PNG lourds | `FullscreenGiftAnimation.tsx` | L.5, L.65–72 | `import Image from 'next/image'` + `fill` + `priority` sur `/gifts/{id}.png.png` |
| Purger résidus Soundboard | `TikTokStyleArena.tsx` | L.88–107 | `SFX_MAP` + `playSfx()` — **plus de bouton Music** (supprimé S.1) |
| Callback SFX réseau résiduel | `TikTokStyleArena.tsx` | L.2745–2747 | `onSfxPlayed` → `playSfx(id)` dans `arenaRealtimeCallbacks` |
| Routage layout vidéo | `ArenaLayoutManager.tsx` | L.131, L.178–214 | `resolveArenaLayoutMode` → `NexusGrid` ou `ConstellationOrbit` |
| Géométrie orbit (hors fichier extrait) | `orbitGeometry.ts` | L.12, L.58, L.82 | Mobile `cH = vpH - 220` ; `centerY: 50` fixe ; `top: calc(cy% + ry*sin(angle) vmin)` |

**Note Phase T :** `ArenaLayoutManager` ne calcule pas `radius` / `centerX` / `centerY` directement — il délègue à `ConstellationOrbit` qui consomme `computeConstellationLayout` (`components/Arena/constellation/orbitGeometry.ts`). Un décalage vertical mobile passera probablement par `centerY` ou `cH` dans ce module satellite.

---

## 1. Code source intégral — `components/Arena/FullscreenGiftAnimation.tsx`
...
## 3. Code source intégral — `components/Arena/ArenaLayoutManager.tsx`

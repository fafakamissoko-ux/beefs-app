# Rapport d'audit — Phase U.0 (Assets 404 + Chat mobile + Focus input)

**Date d'extraction :** 2026-07-16  
**Commit de référence :** `22f359c docs(audit): add Phase T.0 extraction report for arena performance fixes`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse des points d'ancrage Phase U.1

| Objectif U.1 | Fichier | Zone | Détail actuel |
|--------------|---------|------|---------------|
| Corriger 404 next/image | `FullscreenGiftAnimation.tsx` | L.35, L.65–73 | `imageSrc = /gifts/${giftTypeId}.png.png` + `next/image` `unoptimized` |
| Réduire hauteur chat mobile | `TikTokStyleArena.tsx` | L.3420 | Conteneur overlay `max-h-[35dvh]` + `pointer-events-none` |
| Focus input chat | `TikTokStyleArena.tsx` | L.3299, L.3424 | Desktop + mobile : `focus:outline-none` sans `focus:ring` |
| Typographie messages mobile | `ArenaChatMessages.tsx` | L.58–64 | `leading-tight` ; contenu `text-[13px]` ; pseudo `text-[11px]` |
| Cloisonnement pointer-events | `ArenaChatMessages.tsx` | L.48, L.58 | Conteneur mobile `pointer-events-auto` ; parent arène `pointer-events-none` |

**Note U.1 :** Les assets disque sont en `public/gifts/{id}.png.png` (double extension R.1). Une 404 peut venir d'un chemin résolu incorrectement par l'optimiseur Next.js malgré `unoptimized`.

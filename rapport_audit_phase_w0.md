# Rapport d'audit — Phase W.0 (Déconnexion + Scroll chat + img HTML5)
...
## Synthèse des points d'ancrage Phase W

### Fichiers d'authentification identifiés

| Fichier | Rôle | Détail |
|---------|------|--------|
| `contexts/AuthContext.tsx` | **Source** `supabase.auth.signOut()` | L.214–218 : signOut + `window.location.href = '/feed'` (redirect global) |
| `components/Header.tsx` | UI « Déconnexion » | L.649, L.812 : boutons menu appelant `signOut()` |
| `app/settings/page.tsx` | Suppression compte | L.212 : `await signOut()` après delete API (non extrait intégralement) |
| `components/TikTokStyleArena.tsx` | Quitter le direct | `handleLeave` L.2660–2687 — **≠ signOut** (leave Daily / endBeef) |

### Autres cibles W.1

| Objectif | Fichier | Zone | État actuel |
|----------|---------|------|-------------|
| Remplacer next/image | `FullscreenGiftAnimation.tsx` | L.5, L.65–73 | `<Image>` `unoptimized` + `.png.png` |
| Restaurer scroll chat | `ArenaChatMessages.tsx` | L.48 | Mobile conteneur `pointer-events-none` |
| Leave arène | `TikTokStyleArena.tsx` | L.2660–2687 | `stopAllMediaTracks` + `leave()` / `endBeef()` |

**Hypothèse déconnexion globale :** `AuthContext.signOut` force `window.location.href = '/feed'` — toute session Supabase est invalidée + navigation hard. Vérifier déclenchements involontaires (token refresh, 401, double clic Header).
...
## 3. Code source intégral — `contexts/AuthContext.tsx`
...
## 4. Code source intégral — `components/Header.tsx`

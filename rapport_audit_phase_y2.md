# Rapport d'audit — Phase Y.2 (WebP + onLoad + punchlines + recipientName)
...
## Synthèse des points d'ancrage Phase Y.2

| Objectif Y.2 | Fichier | Zone | État actuel |
|--------------|---------|------|-------------|
| Ajouter `recipientName` | `arenaVolatileStore.ts` | L.3–10 | `ArenaBigGiftPayload` sans destinataire |
| Thumbnails .webp picker | `TikTokStyleArena.tsx` | L.3682 | `<span>{gift.emoji}</span>` Unicode |
| Punchline chat | `TikTokStyleArena.tsx` | L.3652 | `msgContent` statique emoji/label |
| Payload FIFO | `TikTokStyleArena.tsx` | L.3663–3671 | `bigPayload` sans `recipientName` / template |
| Asset plein écran | `FullscreenGiftAnimation.tsx` | L.35, L.65–69 | `.png.png` + `<img>` sans `onLoad` |
| Timer 4s | `FullscreenGiftAnimation.tsx` | L.24–30 | Démarre dès `activeGift` — pas gelé au chargement |
| Punchline overlay | `FullscreenGiftAnimation.tsx` | L.79–81 | Texte fixe `{senderName} offre {label} !` |

**Catalogue (satellite) :** `lib/constants/gifts.ts` expose `messageTemplate` avec `{sender}` / `{recipient}` — prêt pour injection Y.2.

# Rapport d'audit — Phase Z.0 (Pilules Glass + messages cadeau)

**Date d'extraction :** 2026-07-17  
**Commit de référence :** `579fbd4 feat(arena): webp gifts, punchlines, and onLoad animation (Phase Y.2)`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse des points d'ancrage Phase Z

| Objectif Z | Fichier | Zone | État actuel |
|------------|---------|------|-------------|
| Discriminant type message | `arenaVolatileStore.ts` | L.14–18 | `VisibleMessage` sans `type` |
| Insertion message | `arenaVolatileStore.ts` | L.33, L.54–58 | `addMessage(msg)` — pas de filtre type |
| Helper envoi chat | `TikTokStyleArena.tsx` | L.641–655 | `addRemoteMessage(name, content, initial?, dbId?)` |
| Envoi cadeau → chat | `TikTokStyleArena.tsx` | L.3654–3660 | `addRemoteMessage(userName, msgContent, …)` punchline Y.2 |
| Réception réseau | `TikTokStyleArena.tsx` | L.2746 | `onMessageReceived` → `addRemoteMessage` |
| Rendu pseudo mobile | `ArenaChatMessages.tsx` | L.60–64 | `span` inline coloré — pas de pilule Glass |
| Rendu pseudo desktop | `ArenaChatMessages.tsx` | L.69–74 | Label uppercase — pas de variante gift |

**Note Z.1 :** tous les messages (texte et cadeau) partagent le même schéma `{ id, user_name, content, initial }` — le discriminant `type?: 'text' \| 'gift'` devra être propagé depuis `addRemoteMessage` / `addMessage` jusqu'à `ArenaChatMessages`.

---

## 1. Code source intégral — `lib/stores/arenaVolatileStore.ts`

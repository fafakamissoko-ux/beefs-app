# Rapport d'audit — Phase Z.1a (Type réseau message + actif money)
...
## Synthèse des points d'ancrage Phase Z.1a

| Objectif Z.1a | Fichier | Zone | État actuel |
|---------------|---------|------|-------------|
| Type message réseau | `useArenaRealtime.ts` | L.66–95 | `ArenaRealtimeCallbacks.onMessageReceived` sans `type` |
| Émission broadcast | `useArenaRealtime.ts` | L.154–159, L.265+ | `broadcastMessage({ user_name, content, initial, id })` |
| Réception broadcast | `useArenaRealtime.ts` | L.471–482 | Handler `message` → `onMessageReceived` sans `type` |
| Polling secours DB | `useArenaRealtime.ts` | L.700+ | `onMessageReceived` depuis Postgres — idem |
| Catalogue cadeaux | `gifts.ts` | L.9–21 | **12 entrées** — pas d'objet `money` |

**Note Z.1a :** ajouter `type?: 'text' \| 'gift'` à `broadcastMessage` args + parser côté `.on('broadcast', { event: 'message' })` + étendre signature `onMessageReceived`.
...
## 2. Code source intégral — `lib/constants/gifts.ts`

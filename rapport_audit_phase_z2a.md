# Rapport d'audit — Phase Z.2a (Asymétrie Mobile/Desktop + Premium Glass)

**Date d'extraction :** 2026-07-17  
**Commit de référence :** `8a74f1e feat(arena): message type over realtime and add Money gift (Phase Z.1a)`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse diagnostic Phase Z.2a

### Asymétrie rendu chat (`ArenaChatMessages.tsx`)

| Branche | Pseudo | Corps message | Pilule Glass | Variante `type: gift` |
|---------|--------|---------------|--------------|----------------------|
| **Mobile** (L.55–66) | `span` inline `text-[11px]` + couleur hash | `span` `text-[12px]` inline | **Non** | **Non** (`msg.type` ignoré) |
| **Desktop** (L.67–75) | `span` block uppercase `text-[9px]` | `div` bulle `rounded-2xl bg-slate-900/40` | **Non** | **Non** |

**Cause asymétrie :** deux arbres JSX distincts sans composant partagé — mobile = flux inline pseudo+texte ; desktop = label au-dessus + bulle glass sur le contenu uniquement.

### Arène post Z.1a (`TikTokStyleArena.tsx`)

| Zone | Ligne | État |
|------|-------|------|
| `addRemoteMessage` | L.641–655 | Signature `(name, content, initial?, dbId?)` — **pas de `type`** |
| Envoi cadeau chat | L.3660 | `addRemoteMessage` sans flag `gift` |
| Broadcast message | L.3661–3666 | `broadcastMessage` sans `type: 'gift'` |
| `onMessageReceived` | L.2746 | Ignore le 6e param `type` du hook |
| **Toast cadeau vert** | **L.3679** | `toast(\`${gift.emoji} ${gift.label} envoyé !\`, 'success')` — **cible suppression Z.2** |

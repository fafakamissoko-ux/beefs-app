# Rapport d'audit — Phase Z.2c (Câblage type: 'gift' + Premium Glass post Z.2)

**Date d'extraction :** 2026-07-17  
**Commit de référence :** `bbe5176 feat(arena): Premium Glass chat UI and remove gift toast (Phase Z.2)`  
**Contrainte :** Zéro modification du code source.

---

## Synthèse diagnostic Phase Z.2c

### Rendu chat (ArenaChatMessages.tsx) — État post Phase Z.2

| Critère | État |
|---------|------|
| Branche unique mobile/desktop | **Oui** — un seul arbre JSX (L.54–78), plus de bifurcation `isMobile ? … : …` |
| Pilule Glass pseudo (texte) | **Oui** — `rounded-full bg-slate-900/60 backdrop-blur-md border border-white/10` (L.69) |
| Texte standard sans fond | **Oui** — `span` flottant sans bulle (L.72–74) |
| Variante `msg.type === 'gift'` | **Oui** — bloc ambré premium (L.57–65) |
| Ombre mobile texte | **Partiel** — `text-shadow` conservé uniquement si `isMobile` (L.72) |

**Verdict UI :** Premium Glass Z.2 **appliqué intégralement** côté composant chat. Le rendu ambré cadeau ne s'activera que si `msg.type === 'gift'` est présent dans le store.

### Arène (TikTokStyleArena.tsx) — Câblage `type: 'gift'`

| Zone | Ligne | État post Z.2 |
|------|-------|---------------|
| `addRemoteMessage` | L.641–655 | Signature `(name, content, initial?, dbId?)` — **pas de paramètre `type`** |
| `addMessage` dans `addRemoteMessage` | L.648–653 | **Pas de champ `type`** injecté |
| Envoi cadeau → chat local | L.3660 | `addRemoteMessage(userName, msgContent, initial, giftKey)` — **sans `type: 'gift'`** |
| Broadcast message cadeau | L.3661–3666 | `broadcastMessage` — **sans `type: 'gift'`** |
| `onMessageReceived` | L.2745–2747 | 4 params seulement — **ignore le 6e param `type` du hook realtime** |
| Toast cadeau vert | — | **Absent** — ligne supprimée en Z.2 (confirmé par grep) |

**Verdict câblage :** asymétrie résiduelle — **UI prête**, **pipeline arène non câblé**. Les messages cadeau arrivent en chat avec `type` undefined → rendu **standard** (pilule pseudo + texte), jamais le bloc ambré.

---

## 1. Code source intégral — `components/TikTokStyleArena.tsx`
...
## 2. Code source intégral — `components/ArenaChatMessages.tsx`

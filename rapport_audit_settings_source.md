# Rapport d'audit source — Page Paramètres (Settings)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** analyser le monolithe `app/settings/page.tsx` et les politiques de sécurité (`lib/password-policy.ts`, `lib/email-signup-policy.ts`) en vue de la refonte **react-hook-form + zod** et de la génération de `lib/schemas/index.ts`.
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `app/settings/page.tsx` | 1377 | Monolithe UI + mutations Supabase |
| `lib/password-policy.ts` | 67 | Politique mot de passe (regex, critères, validation) |
| `lib/email-signup-policy.ts` | 42 | Politique e-mail (domaines jetables, `validateSignupEmail`) |

---

## Synthèse — Navigation par onglets

Type `SettingTab = 'profile' | 'security' | 'wallet' | 'preferences' | 'danger'`  
État : `activeTab` + `setActiveTab(id)` (nav latérale L564–596).

| Onglet | ID | Sections UI rendues | État local principal |
|--------|-----|---------------------|----------------------|
| Profil | `profile` | Informations profil (L600–731) + Bouclier Anti-Spam (L1200–1262) | `profile`, `emailForm`, `emailChangeStatus`, `emailError` |
| Sécurité | `security` | Changement mot de passe + OTP reauth (L734–981) | `passwords`, `passwordStep`, `passwordOtp`, `passwordFieldErrors` |
| Portefeuille | `wallet` | `WithdrawalWizard` + historique transactions (L983–1047) | `walletPoints`, `pointTx` |
| Préférences | `preferences` | Affichage/accessibilité (L1050–1197), Radar & alertes (L1265–1318), Guides (L1321–1340) | `accentColor`, `notifPrefs`, `preferences` (ThemeContext) |
| Zone de danger | `danger` | Suppression compte (L1343–1369) | — |

**Note :** l'onglet Profil affiche **deux cartes** distinctes (profil + bouclier) ; Préférences en affiche **trois** (affichage, notifications, guides).

---

## Synthèse — Chargement initial (`useEffect`)

### `loadProfile` (`useCallback`, L130–161)

Déclenché au mount si `user` présent (L163–175).

| Appel | Table / source |
|-------|----------------|
| `supabase.from('users').select(...).eq('id', user.id).single()` | Table `users` |

→ Hydrate `profile`, `accentColor`, `walletPoints`.

### Préférences notifications (L170–174)

- Lecture `localStorage.getItem('beefs_notif_prefs')` → merge dans `notifPrefs`.

### Historique transactions (L177–191)

| Appel | Table |
|-------|-------|
| `supabase.from('transactions').select(...).limit(50)` | Table `transactions` |

→ **Duplication potentielle** avec la clé react-query `['wallet', user?.id]` sur `app/points/page.tsx` (phase B2).

---

## Synthèse — Mutations Supabase / API

| Handler | Onglet | Flux |
|---------|--------|------|
| `handleSaveProfile` | profile | `users.update({ display_name, bio })` — username **non modifiable** |
| `handleChangeEmail` | profile | 1) `auth.signInWithPassword` (reauth) 2) `auth.updateUser({ email })` — **sans** `validateSignupEmail` |
| `handleUpdatePrivacy` | profile | `users.update({ invitation_privacy })` — enum `everyone | following | nobody` |
| `handleChangePassword` | security | `auth.updateUser({ password, current_password })` ; si `reauthentication_needed` → OTP via `reauthenticate()` + `nonce` |
| `handleResendPasswordOtp` | security | `auth.reauthenticate()` |
| Accent color (inline) | preferences | `users.update({ accent_color })` |
| `toggleNotifPref` | preferences | **localStorage** (`beefs_notif_prefs`) |
| `updatePreferences` | preferences | **ThemeContext** |
| `handleDeleteAccount` | danger | `fetch('/api/account/delete')` + `signOut()` |

---

## Synthèse — Validation actuelle vs politiques `lib/`

| Zone | Validation actuelle | Politique `lib/` utilisée ? |
|------|---------------------|-------------------------------|
| Mot de passe | `validatePasswordPolicy` + blur + `passwordFieldErrors` | **Oui** — `@/lib/password-policy` |
| E-mail (changement) | HTML `type="email"` + reauth | **Non** — `validateSignupEmail` absent |
| Profil | `maxLength` HTML (50 / 200) | Aucun schéma zod |
| Bouclier | Typage TS `InvitationPrivacy` | Aucune validation runtime |

**Implication refonte zod :** wrapper `validatePasswordPolicy` / `validateSignupEmail` dans `.refine()` pour conserver la politique métier inchangée.
...
## Code source — `lib/password-policy.ts`
...
## Code source — `lib/email-signup-policy.ts`

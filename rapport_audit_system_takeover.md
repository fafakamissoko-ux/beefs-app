# Rapport d'audit source — Phase E.1 System Takeover (Dummy Audio WebKit)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** préparer le Hack WebKit — Dummy Audio Track pour widget lock screen iOS + déblocage auto-PiP.

---

## Verdict Architecte — System Takeover E.1

### E.1.1 — Media Session (`useMediaSession.ts`)

| Élément | Statut | Détail |
|---------|--------|--------|
| `MediaMetadata` | ✅ | title, artist, album, artwork L13-21 |
| Handlers `play` / `pause` | ✅ No-op | L27-28 — sans effet réel |
| `playbackState` | ❌ | Jamais `'playing'` |
| Dummy audio / `<audio>` | ❌ | **Aucun contrôleur audio branché** |
| Point d'accroche E.1 | Hook L10-45 | Étendre ici : `startDummyAudio()` + `playbackState` |

**Consommation :** `TikTokStyleArena.tsx` L248-252 — appelé **avant** join, sans geste utilisateur direct sur le hook.

---

### E.1.2 — Routage audio WebRTC (`MeetingAudioOutlet.tsx`)

| Élément | Statut |
|---------|--------|
| Rendu | `<audio>` 1×1px caché par peer distant L31-37 |
| Filtre | Exclut session locale L52-54 |
| `playsInline` | ✅ |
| Media Session | ❌ Non lié au widget OS |
| Dummy track | ❌ Absent |

**Consommation :** `TikTokStyleArena.tsx` L4285 — `<MeetingAudioOutlet peers={physicalPeers} localSessionId={...} />`

**Gap E.1 :** les `<audio>` WebRTC distants ne suffisent pas pour le widget iOS — besoin d'un **dummy silent loop** démarré dans le **geste utilisateur** PreJoin.

---

### E.1.3 — Point d'entrée utilisateur (`PreJoinScreen.tsx`) — Le « clic » WebKit

| Bouton | Ligne | Handler | Geste |
|--------|-------|---------|-------|
| **Rejoindre le direct** | L384-390 | `handleJoin` L160-169 | ✅ Participant — `getUserMedia` déjà appelé L90 |
| **Regarder le Beef** | L203-209 | `onJoin(null)` | ✅ Spectateur |
| Preview cam | L131-138 | `startPreview` au mount | ⚠️ Auto sans clic (iOS) |

**`handleJoin` (L160-169) — point d'injection Dummy Audio :**
```typescript
const handleJoin = () => {
...
**Chaîne parent :**
```
PreJoinScreen.handleJoin
  └─ TikTokStyleArena.handleJoin (L2758) → setShowPreJoin(false) + join Daily
```

**Recommandation E.1 :** démarrer le dummy audio **dans `handleJoin`** (geste explicite) avant `onJoin(...)`, puis synchroniser `useMediaSession.playbackState = 'playing'`.

---

### Synthèse Hack WebKit proposé (non implémenté)

```
Clic « Rejoindre le direct » (PreJoinScreen L386)
  ├─ 1. Créer MediaStream dummy (silent oscillator ou mp3 loop 1 sample)
  ├─ 2. <audio>.play() dans le handler (user gesture)
  ├─ 3. navigator.mediaSession.playbackState = 'playing'
  └─ 4. onJoin → Daily + MeetingAudioOutlet (audio réel distant)
```
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `hooks/useMediaSession.ts` | 47 | Métadonnées lock screen |
| `components/MeetingAudioOutlet.tsx` | 64 | Audio distant `<audio>` |
| `components/PreJoinScreen.tsx` | 396 | **Clic utilisateur** + getUserMedia preview |
...
## `hooks/useMediaSession.ts` (46 lignes)

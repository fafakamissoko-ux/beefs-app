# Rapport d'audit — Wake Lock (Phase G.1)

**Date d'extraction :** 2026-06-28 15:25  
**Branche :** `main` (post F.2 — commit `a044523`)  
**Mission :** Vérification infrastructure Wake Lock + extraction arène pour injection hook.  
...
Objectif : verrouillage d'éveil (`navigator.wakeLock`) pour empêcher la mise en veille (mobile et desktop) pendant les sessions live Agora — participant **et** spectateur.

---

## 1. Vérification d'infrastructure

### Recherche `hooks/useWakeLock.ts` (ou similaire)

**Fichier inexistant, à créer.**

Recherche effectuée dans `hooks/` : aucun fichier `*WakeLock*`, `*wakelock*`, `*wake-lock*` ou `*wake_lock*`.

### Recherche globale codebase

| Critère | Résultat |
|---------|----------|
| `hooks/useWakeLock.ts` | **Absent** |
| Références `wakeLock` / `WakeLock` dans `*.ts` / `*.tsx` | **Aucune** |
| Fichiers nommés `*WakeLock*` / `*wakelock*` (hors node_modules) | **Aucun** |

**Conclusion :** infrastructure Wake Lock **inexistante** — création de `hooks/useWakeLock.ts` requise en G.1.

### API cible (référence W3C)
...
## 2. Arène centrale — `components/TikTokStyleArena.tsx`

**Points d'intérêt Architecte (injection G.1) :**

### Point d'injection recommandé

Niveau racine du composant `TikTokStyleArena`, **immédiatement après** les hooks Tier-1 existants (~l.246–248) :

```typescript
// --- BACKGROUND AUDIO (Tier-1) ---
const { startSystemAudio } = useMediaSession(debateTitle || 'Live Agora', host?.name || 'Ref');
// >>> INJECTION G.1 : useWakeLock() ici <<<
```

**Justification :**
- Le composant monte pour **tous** les rôles (`mediator`, `challenger`, `viewer`, `spectator`)
- Le verrouillage doit couvrir PreJoin + session live (pas seulement post-join Daily)
- Proximité des hooks système (`useMediaSession`) — même couche « Tier-1 »

### États de cycle de vie pertinents

| État | Ligne approx. | Rôle |
|------|---------------|------|
| `export function TikTokStyleArena` | ~227 | Entrée composant |
| `useMediaSession` | ~247 | Metadata + audio système |
| `hasJoined` / `showPreJoin` | ~281–282 | Sas PreJoin vs arène live |
| `isViewer` | ~265 | Spectateur vs participant |
| `useEffect` join Daily | ~1592 | Connexion WebRTC post-join |
| `<PreJoinScreen>` | ~3126 | UI avant entrée room |

### Stratégie G.1 suggérée

1. `useWakeLock({ enabled: true })` au montage du composant arène
2. `release()` au démontage (`return` cleanup du hook)
3. Ré-acquisition sur `visibilitychange` → `visible` (iOS/Android relâchent souvent le lock)
4. Fallback gracieux si `navigator.wakeLock` absent (desktop ancien, Safari partiel)
...
| Élément | Statut |
|---------|--------|
| `hooks/useWakeLock.ts` | **À créer** |
| Usage existant Wake Lock | **Aucun** |
| Fichier d'injection | `components/TikTokStyleArena.tsx` (~l.248) |
| Portée | Participant + spectateur (montage racine arène) |

---

*Fin du rapport — extraction Phase G.1.*

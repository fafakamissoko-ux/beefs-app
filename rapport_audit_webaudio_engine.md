# Rapport d'audit — Web Audio Engine (Phase F.1)

**Date d'extraction :** 2026-06-28 13:00  
**Branche :** `main` (post E.4 — commit `4d78efe`)  
**Mission :** Extraction stricte de la topographie DOM audio / PiP avant remplacement par Web Audio API.  
...
Le hack du lecteur audio fantôme (Base64 + camouflage CSS `#arena-system-audio`) a été rejeté par les heuristiques Apple.  
Objectif F.1 : purger l'override natif E.4 et remplacer le DOM `<audio>` par une architecture Web Audio API rattachée aux composants de l'arène.

---

## 1. Point d'entrée natif — `components/PreJoinScreen.tsx`

**Points d'intérêt Architecte :**
- `useEffect` lignes 140–163 : override natif WebKit (`addEventListener` capture sur `#arena-join-viewer` / `#arena-join-participant`)
- Cible DOM : `document.getElementById('arena-system-audio')` → `play()` + `mediaSession.playbackState`
- Web Audio existant en PreJoin : `AudioContext` + `AnalyserNode` pour le vu-mètre micro (lignes 43–45, 99–114)
- IDs boutons join : `arena-join-viewer` (l.230), `arena-join-participant` (l.414)
...
## 2. Arène et balise fantôme — `components/TikTokStyleArena.tsx`

**Points d'intérêt Architecte :**
- `useMediaSession(debateTitle, host?.name)` — l.247 (metadata lock screen uniquement)
- `<PreJoinScreen>` — ~l.3128
- Balise fantôme `<audio id="arena-system-audio">` — l.4271–4278 (Base64 MPEG, loop, playsInline, camouflage Tailwind)
- `MeetingAudioOutlet` — l.4269 (audio peers distants)
- `playSfx` — l.99–105 (`new Audio()` éphémère pour soundboard)
...
## 3. Contrôleur vidéo (PiP) — `components/ParticipantVideo.tsx`

**Points d'intérêt Architecte :**
- `<video>` : `autoPlay`, `playsInline`, `muted={muted}`, `disablePictureInPicture={false}`
- `usePiP(videoRef, enableAutoPiP)` — auto-PiP conditionnel
- Recyclage `MediaStream` sur `srcObject` (évite clignotement Blink)
- `visibilitychange` → `el.play()` au retour foreground (iOS)
...
## Synthèse topographique (référence rapide)

| Zone | Fichier | Artefact audio/vidéo |
|------|---------|----------------------|
| Sas join (geste utilisateur) | PreJoinScreen | Override natif → `#arena-system-audio.play()` |
| Preview PreJoin | PreJoinScreen | `AudioContext` + analyser (vu-mètre) |
| Racine arène | TikTokStyleArena | `<audio id="arena-system-audio">` camouflé |
| Metadata iOS | TikTokStyleArena | `useMediaSession` |
| Peers distants | TikTokStyleArena | `MeetingAudioOutlet` |
| Tuiles vidéo | ParticipantVideo | `<video>` + `usePiP` |
| SFX régie | TikTokStyleArena | `playSfx` → `new Audio()` |

---

*Fin du rapport — extraction Phase F.1.*

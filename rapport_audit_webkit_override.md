# Rapport d'audit source — Phase E.4 WebKit Override (Vanilla JS)

**Date :** 31 mai 2026  
**Périmètre :** extraction intégrale, zéro modification du code source  
**Objectif :** préparer l'attaque par événement natif pur — contournement perte du jeton User Gesture via React Synthetic Events (E.3).

---

## Verdict Architecte — Échec E.3 Synthetic Events

### Diagnostic iOS Safari

| Couche E.3 | Problème |
|------------|----------|
| `onClick={() => { onTakeSystemFocus(); ... }}` | React SyntheticEvent — WebKit peut ne **pas** propager le user activation token |
| `takeSystemFocusSync` → `audio.play().then(...)` | `.then()` = microtask **hors** geste utilisateur |
| Prop callback React | Indirection supplémentaire vs `addEventListener('click', ..., { capture: true })` |

**Conclusion E.4 :** attacher `play()` sur l'élément `<audio>` via listener **natif** sur le bouton DOM, **première instruction** du handler, sans `.then()` avant `play()` si possible ; `playbackState` peut suivre dans `play()` resolved via listener natif aussi.

---

### E.4.1 — Cibles PreJoin (`PreJoinScreen.tsx`)

| Bouton | Lignes | Handler actuel | ID/ref proposé E.4 |
|--------|--------|----------------|---------------------|
| **👁️ Regarder le Beef** | L205-214 | `onClick` synthetic + `onTakeSystemFocus()` | `id="arena-join-viewer"` ou `ref={viewerJoinBtnRef}` |
| **Rejoindre le direct** | L389-398 | `onClick` synthetic + `onTakeSystemFocus()` + `handleJoin()` | `id="arena-join-participant"` ou `ref={participantJoinBtnRef}` |

**Refs existantes utiles :** `videoRef` L33 (preview), pas de ref sur boutons join.

**Vanilla pattern E.4 proposé :**
```typescript
useEffect(() => {
  const btn = document.getElementById('arena-join-participant');
  if (!btn) return;
  const handler = (e: Event) => {
    const audio = document.getElementById('arena-system-audio') as HTMLAudioElement;
    audio?.play(); // sync, même call stack que le tap natif
    // puis dispatch join...
  };
...
### E.4.2 — Racine arène (`TikTokStyleArena.tsx`)

**`takeSystemFocusSync` — L251-258 :**
```typescript
...
**Prop vers PreJoin — L3139-3144 :**
```tsx
<PreJoinScreen ... onTakeSystemFocus={takeSystemFocusSync} />
```

**Balise `<audio>` fantôme — L4283-4290 :**
```tsx
<audio ref={systemAudioRef} src="data:audio/mpeg;base64,..." loop playsInline hidden aria-hidden />
```

| Attribut actuel | Impact E.4 |
|-----------------|------------|
| `hidden` | Retire du layout ; WebKit peut considérer non-visible |
| Pas de `id` | Vanilla JS ne peut pas cibler sans ref global |
| `volume` via JS 0.01 | OK |

**Modifications visibilité proposées E.4 :**
- Remplacer `hidden` par classes type `fixed w-px h-px opacity-[0.01] pointer-events-none` (composited, dans DOM)
- Ajouter `id="arena-system-audio"` pour accès vanilla document

**Montage :** audio **hors** bloc `showPreJoin` L3137 — persiste après join ✅

---

### E.4.3 — Hook média (`useMediaSession.ts`) — Épuré ✅

| Élément | Statut |
|---------|--------|
| `MediaMetadata` only | ✅ L8-15 |
| Pas de `new Audio()` | ✅ |
| Pas de `takeSystemFocus` | ✅ |
| Cleanup `playbackState = 'none'` | ✅ L21 |
| Conflit E.4 | Aucun — metadata découplée du play |

**Note :** `playbackState = 'playing'` sera posé par le module vanilla E.4, pas par le hook.

---

### Architecture cible E.4 (non implémentée)

```
document.getElementById('arena-join-*').addEventListener('click', nativeHandler, { capture: true })
  └─ nativeHandler (sync):
        1. document.getElementById('arena-system-audio').play()
        2. navigator.mediaSession.playbackState = 'playing' (si play sync OK)
        3. dispatch CustomEvent ou callback join sans onTakeSystemFocus prop
```

Supprimer : prop `onTakeSystemFocus` de PreJoinScreen (remplacée par wiring vanilla ou module `webkitSystemTakeover.ts`).
...
| Fichier | Lignes | Rôle |
|---------|--------|------|
| `components/PreJoinScreen.tsx` | 404 | Boutons join — cibles id/ref |
| `components/TikTokStyleArena.tsx` | ~4392 | audio racine + takeSystemFocusSync |
| `hooks/useMediaSession.ts` | 26 | Metadata only |
...
## `components/PreJoinScreen.tsx` (403 lignes)
...
## `components/TikTokStyleArena.tsx` (4391 lignes)
...
## `hooks/useMediaSession.ts` (25 lignes)

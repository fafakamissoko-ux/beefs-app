-- Phase 2.1 — Exposer beefs_resolved / beefs_abandoned (Indice de Sagesse) en profil public
...
COMMENT ON VIEW public.user_public_profile IS
  'Profils utilisateurs — annuaire sûr (médias, likes, stats Sagesse médiateur).';
...
RETURNS TABLE (
  ...
  beefs_resolved integer,
  beefs_abandoned integer
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    ...
    COALESCE(u.beefs_resolved, 0)::integer,
    COALESCE(u.beefs_abandoned, 0)::integer
  FROM public.users u
...
COMMENT ON FUNCTION public.get_public_profile_by_username(text) IS
  'Profil public par username (anon/auth) — inclut stats Sagesse médiateur.';
